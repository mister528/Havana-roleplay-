...
// ---- palette (blue / marine) ------------------------------------------------
static std::string FishShape(const char* s)
{
                                   ...
                                   unsigned col, unsigned colHover)
{
    ...
    std::string labelSh = FishShape(label);
    ImVec2 tsz = font->CalcTextSizeA(fs, FLT_MAX, 0.0f, labelSh.c_str());
    dl->AddText(font, fs,
        ImVec2(x + (w - tsz.x) * 0.5f, y + (h - tsz.y) * 0.5f),
        kF_Text, labelSh.c_str());
    return clicked;
}
                                    ...
                                    float x, float y, float w)
{
    ...
    ImFont* font = pGUI ? pGUI->GetFont() : ImGui::GetFont();
    // Title
    std::string titleSh = FishShape(title);
    ImVec2 ts = font->CalcTextSizeA(46.0f, FLT_MAX, 0.0f, titleSh.c_str());
    dl->AddText(font, 46.0f, ImVec2(x + 28.0f, y + 16.0f), kF_Text, titleSh.c_str());
    (void)ts;
    if (subtitle && *subtitle) {
        std::string subSh = FishShape(subtitle);
        dl->AddText(font, 24.0f, ImVec2(x + 28.0f, y + 68.0f),
                    kF_TextDim, subSh.c_str());
    }
}
...
void CFishermanOverlay::RenderShop()
{
    ...
    const char* wnTitle = "تطبيق صيد هافانا  -  المارينا";
    ...
    snprintf(vLic, sizeof(vLic),
             m_shopHasLic ? "%d يوم" : "ما فيه", m_shopDaysLeft);
    ...
    SRow rows[5] = {
        ...
        { "ط", "طعم", vBait, kF_Gold },
        { "س", "سنارة",
          m_shopHasRod ? "جاهزة" : "ما فيه",
          m_shopHasRod ? kF_Green : kF_Danger },
        { "$", "فلوس", vMoney, kF_Gold },
        { "ش", "شغل",
          m_shopWorking ? "نشط" : "متوقف",
          m_shopWorking ? kF_Green : kF_TextMuted }
    };
    ...
    dl->AddText(font, 11.0f, ImVec2(wnX + 16, dkY + 12),
                kF_TextMuted,
                "اضغط على قسم للدخول  ·  اكسب $ · رحلة · كرّر");
                                        ...
                                        bool showMoneyChip)
{
    ...
    // Top-left brand pill.
    std::string brandSh = FishShape(brand ? brand : "صيد هافانا");
    const char* brandText = brandSh.c_str();
    ...
    if (brand)
    {
        std::string titleSh = FishShape(brand);
        ImVec2 ts = font->CalcTextSizeA(38.0f, FLT_MAX, 0.0f, titleSh.c_str());
        dl->AddText(font, 38.0f,
                    ImVec2(x + (w - ts.x) * 0.5f, y + 56),
                    kF_Text, titleSh.c_str());
    }
    if (subtitle)
    {
        std::string subSh = FishShape(subtitle);
        ImVec2 ss = font->CalcTextSizeA(18.0f, FLT_MAX, 0.0f, subSh.c_str());
...
void CFishermanOverlay::RenderLicense()
{
    ...
    for (int i = 0; i < (int)m_licOpts.size(); ++i)
    {
        ...
        snprintf(L, sizeof(L), "%d  يوم", m_licOpts[i].days);
        ...
        snprintf(sub, sizeof(sub), "%s / يوم", perDayBuf);
...
void CFishermanOverlay::RenderBait()
{
    ...
    snprintf(subtitleBuf, sizeof(subtitleBuf),
             "$%d للقطعة -- اختر العدد", m_baitUnitPrice);
    ...
    dl->AddText(font, 16.0f, ImVec2(pX + 40, totY + 14),
                kF_TextDim,
                "اضغط +/- عشان تغيّر العدد.");

    // Buy / Back
    if (BigButton("شراء", pX + pW - 260, pY + pH - 110, 220, 70,
...
void CFishermanOverlay::RenderRod()
{
    ...
    DrawPanelFrame(pX, pY, pW, pH,
                   "سنارة الصيد", "سنارة كربون مميزة", true);
    ...
    dl->AddText(font, 18.0f, ImVec2(pX + 40, pY + 192),
                kF_TextDim,
                "السنارة تنتهي بعد إنهاء الرحلة.");

    if (BigButton("شراء", pX + pW - 280, pY + pH - 110, 220, 70,
                  IM_COL32(40, 140, 60, 240), IM_COL32(70, 180, 90, 240)))
        SendOk();
    if (BigButton("رجوع", pX + 40, pY + pH - 90, 220, 60,
                  IM_COL32(60, 60, 60, 240), IM_COL32(100, 100, 100, 240)))
...
void CFishermanOverlay::RenderBoat()
{
    ...
    DrawPanelFrame(pX, pY, pW, pH,
                   "استئجار قارب",
                   "اتجه للمنطقة الحمراء وابدأ الصيد", true);
    ...
    dl->AddText(font, 18.0f, ImVec2(pX + 40, pY + 192),
                kF_TextDim,
                "يرجع للمارينا بعد ما تخلص الرحلة.");
...
void CFishermanOverlay::RenderCast()
{
    ...
    // Header + subtitle
    const char* hdr = "منطقة الصيد";
    const char* sub;
    if (!m_castInZone)        sub = "روح للمنطقة الحمراء على الخريطة";
    else if (!m_castOnBoat)   sub = "اركب القارب وقف داخل المنطقة";
    else if (m_castBait <= 0) sub = "الطعم خلص — ارجع للصياد";
    else                      sub = "السنارة بالماي — استنّى تعضّ...";
    ...
    Chip(pX + 230, pY + 140, "سمك",     cf, kF_Text);
    Chip(pX + 410, pY + 140, "الأرباح",   cm, kF_Green);

    // End button
    if (BigButton("إنهاء الشغل",
...
void CFishermanOverlay::RenderResult()
{
    ...
    TitleStrip(m_rsIsCatch ? "مسكت سمكة!" : "ما طلع شي",
               m_rsIsCatch ? "شدة قوية!" : "حظ أحسن المرة الجاية",
               pX + 16, a.y + 16, pW - 32);
    ...
    if (m_rsIsCatch)
    {
        ...
        // Texts
        char wbuf[32]; snprintf(wbuf, sizeof(wbuf), "%.1f كيلو", m_rsWeight);
    ...
    if (BigButton("إنهاء", pX + 40, a.y + pH - 90, 220, 70,
...
void CFishermanOverlay::RenderStats()
{
    ...
    { std::string _s1 = FishShape("السمك");
    dl->AddText(font, 28.0f, ImVec2(pX + 60, cY + 20), kF_TextDim, _s1.c_str()); }
    dl->AddText(font, 56.0f, ImVec2(pX + 60, cY + 48), kF_Text,   buf);

    snprintf(buf, sizeof(buf), "%.1f كيلو", m_stTotalWeight);
    { std::string _s2 = FishShape("الوزن");
    dl->AddText(font, 28.0f, ImVec2(pX + 340, cY + 20), kF_TextDim, _s2.c_str()); }
    { std::string _bufSh = FishShape(buf);
    dl->AddText(font, 56.0f, ImVec2(pX + 340, cY + 48), kF_Accent, _bufSh.c_str()); }

    char mbuf[48]; FmtMoney(m_stTotalPrice, mbuf, sizeof(mbuf));
    { std::string _s3 = FishShape("الأرباح");
    dl->AddText(font, 28.0f, ImVec2(pX + 680, cY + 20), kF_TextDim, _s3.c_str()); }
    ...
    for (int i = 0; i < (int)m_stRows.size() && i < visible; ++i)
    {
        ...
        dl->AddText(font, 28.0f, ImVec2(pX + 60, ly + 10), kF_Text,
                    r.name.c_str());
        char wbuf[32]; snprintf(wbuf, sizeof(wbuf), "%.1f كيلو", r.weight);
    ...
    if (BigButton("تمام", pX + pW - 260, pY + pH - 90, 220, 70,
                  IM_COL32(40, 140, 60, 240), IM_COL32(70, 180, 90, 240)))
    {
        SendOk();
    }
...
void CFishermanOverlay::RenderMessage()
{
    ...
    if (BigButton("تمام", pX + pW - 240, pY + pH - 90, 200, 70,
                  IM_COL32(40, 140, 60, 240), IM_COL32(70, 180, 90, 240)))
    {
        SendOk();
    }
...
void CFishermanOverlay::RenderHud()
{
    ...
    if (clicked) { FishermanAudio::PlayBite(); StartReelMinigame(); }
}
...
void CFishermanOverlay::RenderReel()
{
    ...
    // Label above bar
    const char* hint = "اضغط بسرعة عشان تسحب";
    ...
    // "TAP!" label big
    const char* tapLbl = (tapped ? "اضغط!" : "اضغط");
    ...
    // Tap counter
    char tpc[32]; snprintf(tpc, sizeof(tpc), "ضغطات: %d", m_reelTaps);
