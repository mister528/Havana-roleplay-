...
{
    // Don't dispatch any snapshot from the filterscript on /inv:
    // the gamemode's CMD:inventory handler is the authoritative source
    // (it sends ~INV_UI~ + island-tool data + ~ACC_UI~). If we also fired
    // an async ~INV_UI~ snapshot here, it would race with the gamemode's
    // payload and -- because this snapshot does NOT carry island-tool
    // fields -- the launcher would wipe the gunpowder/iron/scrap/wood/
    // spring counters a few seconds after the overlay opened (the
    // "appears for a second then disappears" bug).
    //
    // The silent ~INV_DATA~ refresh in InvUI_OnTick() still runs every
    // 4.5s and keeps the cached wallet values fresh; nothing else is
    // needed here.
    #pragma unused cmdtext
    #pragma unused playerid
    return 0; // never consume; let other scripts handle the command
}
...
stock InvUI_DispatchSnapshot(playerid, showOverlay)
{
    ...
    mysql_format(g_inv_sql, query, sizeof(query),
        "SELECT \
            `u_money`,`u_bank`,`u_phohe_balance`,`u_drugs`,`u_cigarette`,\
            ...
            `u_ammo`,`u_guns`,`have_music`,`u_theft_key`,\
            `u_island_gunpowder`,`u_island_iron`,`u_island_scrap`,`u_island_wood`,`u_island_spring`\
...
public InvUI_OnQueryResult(playerid, showOverlay)
{
    ...
    new mats      = 0;
    new cigs      = cache_get_field_content_int(0, "u_cigarette",     g_inv_sql);
    ...
    new music     = cache_get_field_content_int(0, "have_music",      g_inv_sql);
    new theftkey  = cache_get_field_content_int(0, "u_theft_key",     g_inv_sql);

    // Island tool counters - persisted per user, surfaced in the same
    // green /inv overlay alongside the regular consumable items.
    new isl_gp    = cache_get_field_content_int(0, "u_island_gunpowder", g_inv_sql);
    new isl_ir    = cache_get_field_content_int(0, "u_island_iron",     g_inv_sql);
    new isl_sc    = cache_get_field_content_int(0, "u_island_scrap",    g_inv_sql);
    new isl_wd    = cache_get_field_content_int(0, "u_island_wood",     g_inv_sql);
    new isl_sp    = cache_get_field_content_int(0, "u_island_spring",   g_inv_sql);
    ...
    if(showOverlay)
        format(payload, sizeof(payload), prefix_show,
            ...
            money, bank, phone_bal, drugs, mats, cigs);
    else
        format(payload, sizeof(payload), prefix,
            g_inv_pname[playerid], level, job, skin, health, hunger,
            money, bank, phone_bal, drugs, mats, cigs);

    // Note: the island tool fields (gunpowder/iron/scrap/wood/spring) are
    // included so any silent refresh keeps the launcher cache in sync with
    // MySQL after a server-side give/save (HavanaIsland_GiveTool +
    // HavanaIsland_UpdateToolSql in arabonline.pwn).
    format(tail, sizeof(tail),
        "aidkit=%d;bandage=%d;schiene=%d;aspirin=%d;morphine=%d;adrenalin=%d;repairkit=%d;gps=%d;mask=%d;rope=%d;canister=%d;seed=%d;ammo=%d;guns=%d;music=%d;theftkey=%d;gunpowder=%d;iron=%d;scrap=%d;wood=%d;spring=%d",
        aidkit, bandage, schiene, aspirin, morphine, adrenalin,
        repairkit, gps, mask, rope, canister, seed,
        ammo, guns, music, theftkey,
        isl_gp, isl_ir, isl_sc, isl_wd, isl_sp);
