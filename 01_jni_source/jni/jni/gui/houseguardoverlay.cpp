...
{
    if (!font) { dl->AddText(ImVec2(cx, topY), col, txt); return fontSize; }
    ImVec2 sz = font->CalcTextSizeA(fontSize, FLT_MAX, 0.0f, txt);
    dl->AddText(font, fontSize, ImVec2(cx - sz.x * 0.5f, topY), col, txt);
    return sz.y;
}
