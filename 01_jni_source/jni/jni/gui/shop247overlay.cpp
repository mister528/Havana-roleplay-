...
{
    // Badge background
    dl->AddCircleFilled(ImVec2(cx, cy), r, kS_IconBg, 32);
    dl->AddCircle(ImVec2(cx, cy), r, kS_IconRing, 32, 2.5f);

    ImU32 stroke = IM_COL32(255, 255, 255, 240);
...
void CShop247Overlay::Render()
{
    ...
    {
        float iconX = pMin.x + 38.0f * scale;
        float iconY = pMin.y + 38.0f * scale;
        float iS = 64.0f * scale;
    ...
    if (font)
    {
        ...
        const char* t2 = "Tap a product to purchase";

        dl->AddText(font, 42.0f * scale,
                    ImVec2(pMin.x + 130.0f * scale, pMin.y + 24.0f * scale),
                    kS_Text, t1);
        dl->AddText(font, 22.0f * scale,
                    ImVec2(pMin.x + 130.0f * scale, pMin.y + 80.0f * scale),
                    kS_TextDim, t2);

        // Item count pill on the right
        char cb[32];
        snprintf(cb, sizeof(cb), "%d ITEMS", (int)m_items.size());
        float fs = 22.0f * scale;
        ImVec2 tsz = font->CalcTextSizeA(fs, FLT_MAX, 0, cb);
        float padX = 16.0f * scale, padY = 10.0f * scale;
        ImVec2 pillMax(pMax.x - 90.0f * scale, pMin.y + 40.0f * scale + tsz.y);
    ...
    for (size_t i = 0; i < m_items.size(); ++i)
    {
        ...
        {
            // Name
            dl->AddText(font, 26.0f * scale,
                        ImVec2(iconCx + iconR + 16.0f * scale, cMin.y + 22.0f * scale),
                        kS_Text, m_items[i].name.c_str());

            // "Tap to buy" hint
            dl->AddText(font, 16.0f * scale,
                        ImVec2(iconCx + iconR + 16.0f * scale, cMin.y + 58.0f * scale),
                        kS_TextMuted, "Tap to buy");
            ...
            if (ptxt && *ptxt)
            {
                ...
                ImVec2 tsz = font->CalcTextSizeA(fs, FLT_MAX, 0, ptxt);
    ...
    if (font)
    {
        const char* hint = (maxScroll > 0.0f)
            ? "Swipe up or down to see more products"
            : "Tap X to close the market";
