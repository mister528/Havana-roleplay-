                               ...
                               const char* title, const char* body)
{
    ...
    if (s == CARRENT_CARS)
    {
        m_subtitle = "اختر مركبتك";
        ParseItems(body, true);
    }
    else if (s == CARRENT_DURATIONS)
    {
        m_subtitle = "اختر مدة الإيجار";
        ParseItems(body, true);
    }
...
void CCarRentOverlay::Render()
{
    ...
    dl->AddRect(pMin, pMax, kC_Border, rounding, 0, 2.0f * scale);

    // --- Header ---
    float headerH = 130.0f * scale;
    ...
    // Car icon (stylized)
    {
        float iconX = pMin.x + 32.0f * scale;
        float iconY = pMin.y + 30.0f * scale;
        float iconS = 42.0f * scale;
    ...
    // Header title (English, large)
    {
        ...
        if (font)
        {
            float fs = 40.0f * scale;
            dl->AddText(font, fs,
                        ImVec2(pMin.x + 140.0f * scale, pMin.y + 26.0f * scale),
                        kC_Text, title);

            // Subtitle
            dl->AddText(font, 20.0f * scale,
                        ImVec2(pMin.x + 140.0f * scale, pMin.y + 82.0f * scale),
                        kC_TextDim, m_subtitle.c_str());
        }
    }
    ...
    ImVec2 closeC(pMax.x - 44.0f * scale, pMin.y + headerH * 0.5f);
    float  closeR = 24.0f * scale;
    {
        ...
        dl->AddCircleFilled(closeC, closeR,
            hover ? kC_Danger : IM_COL32(40, 60, 50, 230));
        dl->AddCircle(closeC, closeR, kC_BorderDim, 20, 1.6f);
    ...
    if (m_state == CARRENT_CARS || m_state == CARRENT_DURATIONS)
    {
        ...
        float cardH = 120.0f * scale;          // much bigger touch target
        ...
        for (size_t i = 0; i < m_items.size(); ++i)
        {
            ...
            if (inView)
            {
                ...
                // Card bg
                dl->AddRectFilled(cMin, cMax, hover ? kC_CardHover : kC_Card,
                                  14.0f * scale);
                dl->AddRect(cMin, cMax, hover ? kC_Green : kC_BorderDim,
                            14.0f * scale, 0, hover ? 2.0f : 1.2f);
                ...
                // Number badge (larger)
                ImVec2 nC(cMin.x + 60.0f * scale, (cMin.y + cMax.y) * 0.5f);
                dl->AddCircleFilled(nC, 34.0f * scale, IM_COL32(0, 60, 40, 230));
                dl->AddCircle(nC, 34.0f * scale, kC_Green, 32, 2.0f);
                char nbuf[8];
                snprintf(nbuf, sizeof(nbuf), "%d", (int)i + 1);
                if (font)
                {
                    ImVec2 tsz = font->CalcTextSizeA(32.0f * scale, FLT_MAX, 0, nbuf);
                    dl->AddText(font, 32.0f * scale,
                ...
                if (font)
                {
                    dl->AddText(font, 34.0f * scale,
                        ImVec2(cMin.x + 110.0f * scale, cMin.y + 24.0f * scale),
                        kC_Text, m_items[i].label.c_str());

                    // Small Arabic sub-label under the car name
                    dl->AddText(font, 18.0f * scale,
                        ImVec2(cMin.x + 110.0f * scale, cMin.y + 70.0f * scale),
                ...
                if (pricetxt && *pricetxt)
                {
                    ...
                    ImVec2 pillMin(pillMax.x - tsz.x - padX * 2.0f,
                                   (cMin.y + cMax.y) * 0.5f - tsz.y * 0.5f - padY);
                    dl->AddRectFilled(pillMin, pillMax, kC_PriceBg, 14.0f * scale);
                    dl->AddRect(pillMin, pillMax, kC_Green, 14.0f * scale, 0, 1.2f);
                    if (font)
                        dl->AddText(font, fs,
        ...
        if (font)
        {
            const char* hint = (m_state == CARRENT_CARS)
                ? "اسحب فوق أو تحت لعرض مركبات أكثر"
                : "اختر مدة الإيجار المناسبة لك";
            float fs = 20.0f * scale;
            ImVec2 tsz = font->CalcTextSizeA(fs, FLT_MAX, 0, hint);
            dl->AddText(font, fs,
    ...
    else if (m_state == CARRENT_PREVIEW || m_state == CARRENT_CONFIRM)
    {
        ...
        float cardH = panelH - headerH - 130.0f * scale;
        float x0 = pMin.x + 40.0f * scale;
        float y0 = contentTop + 10.0f * scale;
        ImVec2 cMin(x0, y0);
        ImVec2 cMax(x0 + cardW, y0 + cardH);

        dl->AddRectFilled(cMin, cMax, kC_BgInner, 16.0f * scale);
        dl->AddRect(cMin, cMax, kC_BorderDim, 16.0f * scale, 0, 1.4f);

        // Draw each line of confirm body on its own row
        if (font)
        {
            float lineY = cMin.y + 26.0f * scale;
        ...
        dl->AddRectFilledMultiColor(confMin, confMax,
            confPressing ? IM_COL32(0, 255, 160, 255) : IM_COL32(0, 220, 140, 240),
            confPressing ? IM_COL32(40, 255, 180, 255) : IM_COL32(17, 180, 80, 240),
            confPressing ? IM_COL32(20, 210, 120, 255) : IM_COL32(0, 150, 80, 240),
            confPressing ? IM_COL32(0, 230, 140, 255) : IM_COL32(0, 190, 110, 240));
        dl->AddRect(confMin, confMax, kC_Green, 12.0f * scale, 0, 2.0f);
        if (font)
        {
            float fs = 22.0f * scale;
            ImVec2 tsz = font->CalcTextSizeA(fs, FLT_MAX, 0, primaryLabel);
            dl->AddText(font, fs,
                ImVec2((confMin.x + confMax.x) * 0.5f - tsz.x * 0.5f,
                       (confMin.y + confMax.y) * 0.5f - tsz.y * 0.5f),
                IM_COL32(10, 40, 20, 255), primaryLabel);
        }

        // Secondary button
        dl->AddRectFilled(backMin, backMax,
            backPressing ? IM_COL32(80, 80, 80, 240) : IM_COL32(50, 60, 55, 240),
            12.0f * scale);
        dl->AddRect(backMin, backMax, kC_BorderDim, 12.0f * scale, 0, 1.6f);
        if (font)
        {
            float fs = 22.0f * scale;
            ImVec2 tsz = font->CalcTextSizeA(fs, FLT_MAX, 0, secondaryLabel);
            dl->AddText(font, fs,
                ImVec2((backMin.x + backMax.x) * 0.5f - tsz.x * 0.5f,
                       (backMin.y + backMax.y) * 0.5f - tsz.y * 0.5f),
                kC_Text, secondaryLabel);
        }
