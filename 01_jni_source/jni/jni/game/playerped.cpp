...
// Remember the last interior we've been told to be in so ScrSetPlayerPos can
// re-apply it after a teleport. Without re-applying, teleports into non-zero
// interiors can leave the camera in the previous cell and no world meshes
// streamed in at the new (X,Y) — the character appears to fall into the sky.
uint8_t g_byteLastLocalInterior = 0;
...
void CPlayerPed::SetInterior(uint8_t byteID)
{
	...
	g_byteLastLocalInterior = byteID;
...
// Re-apply the active interior + streaming refresh at (x, y). Called after a
// teleport so the engine loads meshes for the current interior at the new
// coordinates. Finishes with load_requested_models (0x038B) which forces
// the streamer to synchronously bring in the models refresh_streaming_at
// just requested — without this, some interiors (notably 24/7 / int 18)
// get stuck with their meshes on the pending queue and the player falls
// through an empty cell into the skybox.
void CPlayerPed::ReapplyInteriorAt(float x, float y)
{
	if(!m_pPed) return;
	if (!GamePool_Ped_GetAt(m_dwGTAId)) return;

	ScriptCommand(&select_interior, g_byteLastLocalInterior);
	ScriptCommand(&link_actor_to_interior, m_dwGTAId, g_byteLastLocalInterior);
