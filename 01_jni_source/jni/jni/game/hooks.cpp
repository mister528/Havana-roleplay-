...
static inline bool havana_text_safe(const uint16_t* text)
{
	...
	for (int i = 0; i < 4096; ++i) {
		const uint16_t* p = text + i;
		uintptr_t page = ((uintptr_t)p) & ~(uintptr_t)(page_size - 1);
		if (page != cur_page) {
			if (mincore((void*)page, page_size, &vec) != 0)
				return false; // page not mapped - bail before SIGSEGV
			cur_page = page;
		}
		if (*p == 0)
			return true; // found null terminator within bound
	}
	return false; // unterminated string within 4 KB - reject as garbage
}
