...
void ApplyPatches()
{
	...
	// libGTASA stores the texture-name strings "splash1"/"splash2"/"splash3"
	// in .rodata at +0x5b3c0c/+0x5b3c14/+0x5b3c1c, and CMenuManager looks
	// them up via TextureDatabaseRuntime::GetTexture. By writing a single
	// 0x00 byte at the start of each string we turn them into "" so the
	// lookups fail and the slots render as solid black background.
	{
		static const char zero = 0;
		WriteMemory(g_libGTASA + 0x5B3C0C, (uintptr_t)&zero, 1); // "splash1"->""
		WriteMemory(g_libGTASA + 0x5B3C14, (uintptr_t)&zero, 1); // "splash2"->""
		WriteMemory(g_libGTASA + 0x5B3C1C, (uintptr_t)&zero, 1); // "splash3"->""
		Log("[HavanaPauseArt] splash1/2/3 strings nulled in .rodata");
	}
