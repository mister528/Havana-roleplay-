...
void CDialogWindow::SetInfo(char* szInfo, int length)
{
	...
	// [HARDEN] cp1251 → UTF-8 expansion is up to 3 bytes per input byte; the
	// allocation is (length * 3) + 1 so the safe input cap is `length`.
	// Without this, an unterminated server-side info could expand past the
	// allocated heap chunk, smash adjacent allocations, and trap GTA-SA
	// later inside strncpy / CFont::GetNextSpace when the dialog rendered.
	cp1251_to_utf8(m_putf8Info, szInfo, (unsigned int)length);
	m_putf8Info[(length * 3)] = '\0';

	// =========
	char szNonColorEmbeddedMsg[4200];
	const size_t kSafeMax = sizeof(szNonColorEmbeddedMsg) - 1;
	size_t iNonColorEmbeddedMsgLen = 0;

	// [HARDEN] Cap the strip-color loop at sizeof(szNonColorEmbeddedMsg) - 1.
	// The previous loop trusted strlen(szInfo) blindly; a long server-side
	// info with no embedded color tokens would overflow the 4200-byte stack
	// buffer.
	const size_t infoLen = strlen(szInfo);
	for (size_t pos = 0; pos < infoLen && szInfo[pos] != '\0'; pos++)
	{
		if (pos + 7 < infoLen)
		{
		...
		if (iNonColorEmbeddedMsgLen >= kSafeMax) break;
	...
	// [HARDEN] Same bound as m_putf8Info above.
	cp1251_to_utf8(m_pszInfo, szNonColorEmbeddedMsg, (unsigned int)length);
	m_pszInfo[(length * 3)] = '\0';
}
...
{
	if (!str || *str == '\0') return;
	// [HARDEN] szDialogInputBuffer is 100 bytes and utf8DialogInputBuffer is
	// 100*3 bytes; the previous unchecked strcpy / unbounded cp1251_to_utf8
	// would overflow the stack/.bss when the keyboard handed back a long
	// edited string (Arabic IME composition or pasted text).
	{
		size_t n = 0;
		while (str[n] && n < 99)
		{
			szDialogInputBuffer[n] = str[n];
			++n;
		}
		szDialogInputBuffer[n] = '\0';
	}
	cp1251_to_utf8(utf8DialogInputBuffer, str, 99);
	utf8DialogInputBuffer[(100 * 3) - 1] = '\0';
}
