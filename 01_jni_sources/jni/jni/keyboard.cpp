...
void CKeyBoard::Render()
{
	...
	if (IsHidden())
	{
		...
		ImGui::GetOverlayDrawList()->AddText(pGUI->GetFont(), m_fFontSize,
			ImVec2(m_Pos.x + m_Size.x * 0.02, m_Pos.y + m_Pos.y * 0.05), 0xFFFFFFFF,
			_utf8DialogInputBuffer);
	}
	else
	{
		// ImDrawList::AddText already shapes Arabic; pass raw bytes.
	...
	CmdSuggest::Render(m_Pos, m_Size, m_fFontSize, m_sInput);
}
...
bool CKeyBoard::OnTouchEvent(int type, bool multi, int x, int y)
{
	...
	if (CmdSuggest::HandleTouch(type, x, y, m_Pos, m_Size, m_fFontSize, m_sInput, m_utf8Input, m_iInputOffset))
		return false;
...
void CKeyBoard::HandleInput(kbKey& key)
{
	...
	case KEY_DEFAULT:
	case KEY_SPACE:
		if (key.utf8push[m_iCase][0])
		{
			for (int _i = 0; _i < 7 && key.utf8push[m_iCase][_i]; ++_i)
				AddCharToInput(key.utf8push[m_iCase][_i]);
		}
		else
		{
	...
	case KEY_SWITCH:
		// Custom cycle requested by user: ENG -> ARA -> NUM -> RUS -> ENG
		switch (m_iLayout)
		{
		case LAYOUT_ENG: m_iLayout = LAYOUT_ARA; break;
		case LAYOUT_ARA: m_iLayout = LAYOUT_NUM; break;
		case LAYOUT_NUM: m_iLayout = LAYOUT_ENG; break;
		default:         m_iLayout = LAYOUT_ENG; break;
		}
		m_iCase = LOWER_CASE;
		break;
...
void CKeyBoard::RebuildInputEcho(int offsetDelta)
{
	int base = m_iInputOffset + offsetDelta;
	if (base < 0) base = 0;
	const unsigned char* src = (const unsigned char*)(m_sInput.c_str() + base);
	size_t srcLen = m_sInput.size() > (size_t)base ? m_sInput.size() - base : 0;

	// Heuristic: scan for a valid UTF-8 multibyte sequence whose decoded
	// codepoint is in the Arabic blocks. If we find one we treat the entire
	// buffer as already UTF-8 and copy it verbatim. Otherwise we fall back
	// to the original CP1251 -> UTF-8 transcoder so Russian / Cyrillic input
	// keeps working exactly as before.
	bool isUtf8Arabic = false;
	for (size_t i = 0; i < srcLen; )
	{
		unsigned char c = src[i];
		if (c < 0x80) { ++i; continue; }
		if ((c & 0xE0) == 0xC0 && i + 1 < srcLen && (src[i+1] & 0xC0) == 0x80)
		{
			unsigned int cp = ((c & 0x1F) << 6) | (src[i+1] & 0x3F);
			if (cp >= 0x0600 && cp <= 0x06FF) { isUtf8Arabic = true; break; }
			i += 2;
		}
		else if ((c & 0xF0) == 0xE0 && i + 2 < srcLen) { i += 3; }
		else if ((c & 0xF8) == 0xF0 && i + 3 < srcLen) { i += 4; }
		else { ++i; }
	}
	...
	if (isUtf8Arabic)
	{
		...
		if (copyLen >= sizeof(m_utf8Input)) copyLen = sizeof(m_utf8Input) - 1;
		memcpy(m_utf8Input, src, copyLen);
		m_utf8Input[copyLen] = 0;
	}
	else
	{
		cp1251_to_utf8(m_utf8Input, (const char*)src);
	}
}
...
void CKeyBoard::DeleteCharFromInput()
{
	...
	ImVec2 textSize;
	// Pop one full UTF-8 codepoint, not one byte. If the trailing byte is a
	// continuation byte (10xxxxxx) keep popping until we land on either an
	// ASCII byte (0xxxxxxx) or a UTF-8 leading byte (11xxxxxx), then pop
	// that too. This prevents leaving a half-encoded multibyte sequence
	// (which renders as garbage and corrupts the message when sent).
	while (!m_sInput.empty())
	{
		unsigned char _b = (unsigned char)m_sInput.back();
		m_sInput.pop_back();
		if (_b < 0x80) break;
		if ((_b & 0xC0) == 0xC0) break;
	}
...
void CKeyBoard::InitARA()
{
	Log(__FUNCTION__);
	ImVec2 curPos;
	std::vector<kbKey>* row = nullptr;
	std::vector<kbKey>::iterator it;
	float defWidth = m_Size.x / 10.0f;
	...
	// Row 0: 10 letters - first 10 of MS-Arabic 101 top row.
	// All letters stored as their Unicode codepoint encoded as UTF-8 bytes.
	const char* row0[10] = {
		"\xD8\xB6", // U+0636 DAD
		"\xD8\xB5", // U+0635 SAD
		"\xD8\xAB", // U+062B THEH
	...
	row = &m_Rows[LAYOUT_ARA][0];
	curPos = ImVec2(0, m_Pos.y + m_fKeySizeY);
	for (int i = 0; i < 10; ++i)
	{
		key.pos = curPos;
		key.symPos = ImVec2(curPos.x + defWidth * 0.4f, curPos.y + m_fKeySizeY * 0.2f);
		key.width = defWidth;
		key.code[LOWER_CASE] = 0;
		key.code[UPPER_CASE] = 0;
		strncpy(key.name[LOWER_CASE], row0[i], 7); key.name[LOWER_CASE][7] = 0;
		strncpy(key.name[UPPER_CASE], row0[i], 7); key.name[UPPER_CASE][7] = 0;
		strncpy(key.utf8push[LOWER_CASE], row0[i], 7); key.utf8push[LOWER_CASE][7] = 0;
		strncpy(key.utf8push[UPPER_CASE], row0[i], 7); key.utf8push[UPPER_CASE][7] = 0;
		key.id++;
		it = row->begin();
		row->insert(it, key);
		curPos.x += key.width;
	...
	for (int i = 0; i < 10; ++i)
	{
		...
		key.code[LOWER_CASE] = 0;
		key.code[UPPER_CASE] = 0;
		strncpy(key.name[LOWER_CASE], row1[i], 7); key.name[LOWER_CASE][7] = 0;
		strncpy(key.name[UPPER_CASE], row1[i], 7); key.name[UPPER_CASE][7] = 0;
		strncpy(key.utf8push[LOWER_CASE], row1[i], 7); key.utf8push[LOWER_CASE][7] = 0;
		strncpy(key.utf8push[UPPER_CASE], row1[i], 7); key.utf8push[UPPER_CASE][7] = 0;
		key.id++;
		it = row->begin();
		row->insert(it, key);
		curPos.x += key.width;
	...
	// Row 2: 8 letters + backspace at the right end
	const char* row2[8] = {
		"\xD8\xB1", // U+0631 REH
		"\xD8\xB2", // U+0632 ZAIN
		"\xD9\x88", // U+0648 WAW
	...
	row = &m_Rows[LAYOUT_ARA][2];
	curPos.x = defWidth; // leave one slot on the left for symmetry / shift area
	curPos.y += m_fKeySizeY;
	for (int i = 0; i < 8; ++i)
	{
		key.pos = curPos;
		...
		key.code[LOWER_CASE] = 0;
		key.code[UPPER_CASE] = 0;
		strncpy(key.name[LOWER_CASE], row2[i], 7); key.name[LOWER_CASE][7] = 0;
		strncpy(key.name[UPPER_CASE], row2[i], 7); key.name[UPPER_CASE][7] = 0;
		strncpy(key.utf8push[LOWER_CASE], row2[i], 7); key.utf8push[LOWER_CASE][7] = 0;
		strncpy(key.utf8push[UPPER_CASE], row2[i], 7); key.utf8push[UPPER_CASE][7] = 0;
		key.id++;
		it = row->begin();
		row->insert(it, key);
		curPos.x += key.width;
	}
	// Backspace at the right edge of row 2
	memset(&key.utf8push, 0, sizeof(key.utf8push));
	key.pos = curPos;
	key.symPos = ImVec2(curPos.x + defWidth * 0.2f, curPos.y + m_fKeySizeY * 0.2f);
	key.width = defWidth;
	key.code[LOWER_CASE] = 0;
	key.code[UPPER_CASE] = 0;
	key.name[LOWER_CASE][0] = 0;
	...
	// Row 3: control row mirrors ENG layout: / , <lang> [space] ? ! send
	row = &m_Rows[LAYOUT_ARA][3];
	curPos.x = 0;
	curPos.y += m_fKeySizeY;
	key.type = KEY_DEFAULT;
	memset(&key.utf8push, 0, sizeof(key.utf8push));
	...
	key.code[LOWER_CASE] = '/';
	key.code[UPPER_CASE] = '/';
	strcpy(key.name[LOWER_CASE], "/");
	strcpy(key.name[UPPER_CASE], "/");
	key.id++;
	it = row->begin();
	row->insert(it, key);
	curPos.x += key.width;
