...
bool Arabic::ContainsArabic(const char* utf8, int len)
{
        ...
        {
                uint32_t cp = cps[i];
                // Vowel marks (fatha/damma/kasra/...) and Quranic
                // annotations: combining marks, transparent in BiDi.
                if (cp >= 0x064B && cp <= 0x065F) continue;
                if (cp == 0x0670) continue;
                if (cp >= 0x06D6 && cp <= 0x06ED) continue;
                // Other non-shapable codepoints in the Arabic block:
                // Arabic signs, BiDi marks (ALM), punctuation (؟ ، ؛
                // ؞ ٪ ٬ ٫ ٭), Arabic-Indic digits and tatweel. They
                // survive Shape() unchanged, so the visual-order
                // output of an already-shaped paragraph still has
                // them in the 0x06xx range. Treating them as
                // "Arabic" here would make ImDrawList::AddText call
                // Shape() a second time on visual-ordered text and
                // BiDi-reverse the run order back to logical, which
                // is exactly the corruption symptom users see when
                // a message ends with "؟" or any punctuation mark.
                if (cp <= 0x061F) continue;
                if (cp == 0x0640) continue;
                if (cp >= 0x0660 && cp <= 0x066D) continue;
                if (cp >= 0x06F0 && cp <= 0x06F9) continue;
...
namespace
{
        ...
        auto isLatinLetter = [](uint32_t cp) {
        ...
        size_t countR = 0, countLatin = 0;
        for (size_t k = 0; k < N; ++k)
        {
                if      (isStrongRtl(ligatured[k])) { dir[k] = 'R'; ++countR; }
                else if (isStrongLtr(ligatured[k]))
                {
                        dir[k] = 'L';
                        if (isLatinLetter(ligatured[k])) ++countLatin;
                }
        }
        ...
        // the all-Latin rows ("- hi (ahmadgatga) [0]"). Digits are still drawn
        // LTR as their own runs, but they do not decide the paragraph base;
        // Arabic shop/chat messages with "$10,000" or "24/7" must remain RTL.
        const char paraDir = (countLatin > countR) ? 'L' : 'R';
