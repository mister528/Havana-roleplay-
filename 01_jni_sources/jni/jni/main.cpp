...
#include "gui/gametextoverlay.h"
#include "gui/shop247overlay.h"
#include "gui/inventoryoverlay.h"
#include "gui/skinshopoverlay.h"
...
#include "gui/brandoverlay.h"
#include "gui/test1overlay.h"
...
#include "gui/greenzoneoverlay.h"
...
const cryptor::string_encryptor encLib = cryptor::create("luxury.so", 11);
...
static void ForceFreeAimTargeting()
{
	if(!g_libGTASA) return;

	uintptr_t* targetOptions = (uintptr_t*)(g_libGTASA + 0x5DE3E8);
	uintptr_t mobFree = g_libGTASA + 0x596D84;
	targetOptions[0] = mobFree;
	targetOptions[1] = mobFree;

	int* targetSetup = (int*)(g_libGTASA + 0x5DDF58 + 6 * 28);
	targetSetup[0] = 1;
	targetSetup[1] = 1;
	targetSetup[2] = 1;

	uintptr_t targetSetting = g_libGTASA + 0x610F10 + 6 * 32;
	*(int*)(targetSetting) = 1;
	*(uintptr_t*)(targetSetting + 4) = (uintptr_t)targetOptions;
	*(int*)(targetSetting + 8) = 1;
	*(int*)(targetSetting + 16) = 1;
}
...
void InitSAMP(JNIEnv* pEnv, jobject thiz)
{
	...
	g_pszStorage = "/storage/emulated/0/LuxuryMobile/";
	...
	// Ship the bundled vehicle-patch files (gta3.img with Azelow over the
	// STALLION slot + an upgraded handling.cfg) out to the launcher's
	// storage on first run. Subsequent launches see the marker file and
	// skip. The NvFOpen hooks then redirect the game's reads to these
	// files automatically.
	AzelowEnsureAssetsInstalled(pEnv, thiz, g_pszStorage);
...
void InitInMenu()
{
	...
	pBankOverlay = new CBankOverlay();
	pCarRentOverlay = new CCarRentOverlay();
	pTrentPreviewOverlay = new CTrentPreviewOverlay();
	pGameTextOverlay = new CGameTextOverlay();
	pShop247Overlay = new CShop247Overlay();
	pInventoryOverlay = new CInventoryOverlay();
	pSkinShopOverlay  = new CSkinShopOverlay();
	pClothesShopOverlay = new CClothesShopOverlay();
	pAccessoryShopOverlay = new CAccessoryShopOverlay();
	pCarShopOverlay       = new CCarShopOverlay();
	pHouseGuardOverlay = new CHouseGuardOverlay();
	pBrandOverlay = new CBrandOverlay();
	pTest1Overlay = new CTest1Overlay();
	pTest3Overlay = new CTest3Overlay();
	pFlightJobOverlay = new CFlightJobOverlay();
	pFishermanOverlay = new CFishermanOverlay();
	pYtMusicOverlay = new CYtMusicOverlay();
	pRobberyOverlay = new CRobberyOverlay();
	pCarJackOverlay = new CCarJackOverlay();
	pPoliceHudOverlay = new CPoliceHudOverlay();
	pWantedOverlay = new CWantedOverlay();
	pTestInvOverlay = new CTestInvOverlay();
	pFlyModsOverlay = new CFlyModsOverlay();
	pGreenZoneOverlay = new CGreenZoneOverlay();
	pPhoneOverlay = new CPhoneOverlay();
	pObjectEditorOverlay = new CObjectEditorOverlay();
	pIslandToolsOverlay = new CIslandToolsOverlay();
	pWeaponCraftOverlay = new CWeaponCraftOverlay();
...
/*
		pNetGame = new CNetGame(cryptor::create("141.94.184.108", 12).decrypt(), atoi(cryptor::create("1241", 4).decrypt()), pSettings->GetReadOnly().szNickName, pSettings->GetReadOnly().szPassword);
*/
...
{
	// process pause event - but do NOT actually pause game loop
	// so RakNet keeps sending packets and player stays connected (AFK mode)
	if (g_pJavaWrapper)
	{
		if (pKeyBoard)
		{
	...
	// DO NOT set pause flag - keep game loop and RakNet alive
...
void MainLoop()
{
	...
	// [FIX] Keep the vanilla SA-MP HUD (money/weapon/ammo/wanted) visible
	// regardless of server requests. Some RP servers set CTheScripts::
	// bDisplayHud = 0 to hide the stock HUD; user wants it always on.
