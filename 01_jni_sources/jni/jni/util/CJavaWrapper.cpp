...
void CJavaWrapper::SetClipboardString(const char* text)
{
	JNIEnv* env = GetEnv();
	if (!env || !text) return;

	jclass clsContext = env->FindClass("android/content/Context");
	jfieldID fidClipboard = clsContext ? env->GetStaticFieldID(clsContext, "CLIPBOARD_SERVICE", "Ljava/lang/String;") : nullptr;
	jobject serviceName = fidClipboard ? env->GetStaticObjectField(clsContext, fidClipboard) : nullptr;
	jclass clsActivity = env->GetObjectClass(activity);
	jmethodID midGetSystemService = clsActivity ? env->GetMethodID(clsActivity, "getSystemService", "(Ljava/lang/String;)Ljava/lang/Object;") : nullptr;
	jobject clipboard = (midGetSystemService && serviceName) ? env->CallObjectMethod(activity, midGetSystemService, serviceName) : nullptr;
	jclass clsClipData = env->FindClass("android/content/ClipData");
	jmethodID midNewPlainText = clsClipData ? env->GetStaticMethodID(clsClipData, "newPlainText", "(Ljava/lang/CharSequence;Ljava/lang/CharSequence;)Landroid/content/ClipData;") : nullptr;
	jstring label = env->NewStringUTF("Phone ID");
	jstring value = env->NewStringUTF(text);
	jobject clip = (midNewPlainText && label && value) ? env->CallStaticObjectMethod(clsClipData, midNewPlainText, label, value) : nullptr;
	jclass clsClipboard = env->FindClass("android/content/ClipboardManager");
	jmethodID midSetPrimaryClip = clsClipboard ? env->GetMethodID(clsClipboard, "setPrimaryClip", "(Landroid/content/ClipData;)V") : nullptr;
	if (clipboard && clip && midSetPrimaryClip)
		env->CallVoidMethod(clipboard, midSetPrimaryClip, clip);
	...
	if (clsClipboard) env->DeleteLocalRef(clsClipboard);
	if (clsClipData) env->DeleteLocalRef(clsClipData);
	if (clsActivity) env->DeleteLocalRef(clsActivity);
	if (clsContext) env->DeleteLocalRef(clsContext);
}
...
void CJavaWrapper::UpdateHudInfo(int health, int kaban, int armour, int hunger, int weaponid, int ammo, int ammoinclip, int money, int wanted)
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_updateHudInfo, health, kaban, armour, hunger, weaponid, ammo, ammoinclip, money, wanted);
}


// AFK wake lock: keep CPU alive when user pauses (minimizes) the app
static jobject g_afkWakeLock = nullptr;
static void SetAfkWakeLock(JNIEnv* env, jobject activity, bool pause)
{
    if (pause)
    {
        if (g_afkWakeLock)
        {
            jclass clsWL = env->FindClass("android/os/PowerManager$WakeLock");
            if (clsWL)
            {
                jmethodID midAcq = env->GetMethodID(clsWL, "acquire", "(J)V");
                if (midAcq) env->CallVoidMethod(g_afkWakeLock, midAcq, (jlong)600000);
                env->DeleteLocalRef(clsWL);
            }
            if (env->ExceptionCheck()) { env->ExceptionDescribe(); env->ExceptionClear(); }
            return;
        }
        jclass clsCtx = env->FindClass("android/content/Context");
        if (!clsCtx) { if (env->ExceptionCheck()) { env->ExceptionDescribe(); env->ExceptionClear(); } return; }
        jfieldID fidPS = env->GetStaticFieldID(clsCtx, "POWER_SERVICE", "Ljava/lang/String;");
        jobject svcName = fidPS ? env->GetStaticObjectField(clsCtx, fidPS) : nullptr;
        jmethodID midGSS = env->GetMethodID(env->GetObjectClass(activity), "getSystemService", "(Ljava/lang/String;)Ljava/lang/Object;");
        jobject pm = (midGSS && svcName) ? env->CallObjectMethod(activity, midGSS, svcName) : nullptr;
        if (!pm) { if (env->ExceptionCheck()) { env->ExceptionDescribe(); env->ExceptionClear(); } return; }
        jclass clsPM = env->FindClass("android/os/PowerManager");
        jmethodID midNWL = clsPM ? env->GetMethodID(clsPM, "newWakeLock", "(ILjava/lang/String;)Landroid/os/PowerManager$WakeLock;") : nullptr;
        if (!midNWL) { if (env->ExceptionCheck()) { env->ExceptionDescribe(); env->ExceptionClear(); } return; }
        jstring tag = env->NewStringUTF("HavanaRp:AFKKeepAlive");
        jobject wl = env->CallObjectMethod(pm, midNWL, (jint)1, tag);
        if (!wl) { if (env->ExceptionCheck()) { env->ExceptionDescribe(); env->ExceptionClear(); } return; }
        jclass clsWL = env->FindClass("android/os/PowerManager$WakeLock");
        jmethodID midRef = clsWL ? env->GetMethodID(clsWL, "setReferenceCounted", "(Z)V") : nullptr;
        if (midRef) env->CallVoidMethod(wl, midRef, (jboolean)JNI_FALSE);
        jmethodID midAcq = clsWL ? env->GetMethodID(clsWL, "acquire", "(J)V") : nullptr;
        if (midAcq) env->CallVoidMethod(wl, midAcq, (jlong)600000);
        g_afkWakeLock = env->NewGlobalRef(wl);
        if (env->ExceptionCheck()) { env->ExceptionDescribe(); env->ExceptionClear(); }
    }
    else
    {
        if (g_afkWakeLock)
        {
            ...
            if (clsWL)
            {
                jmethodID midHeld = env->GetMethodID(clsWL, "isHeld", "()Z");
                if (midHeld && env->CallBooleanMethod(g_afkWakeLock, midHeld))
                {
                    jmethodID midRel = env->GetMethodID(clsWL, "release", "()V");
                    if (midRel) env->CallVoidMethod(g_afkWakeLock, midRel);
                }
                env->DeleteLocalRef(clsWL);
            }
            env->DeleteGlobalRef(g_afkWakeLock);
            g_afkWakeLock = nullptr;
            if (env->ExceptionCheck()) { env->ExceptionDescribe(); env->ExceptionClear(); }
        }
    }
}
...
void CJavaWrapper::SetPauseState(bool a1)
{
    ...
    SetAfkWakeLock(env, this->activity, a1);
...
CJavaWrapper::CJavaWrapper(JNIEnv* env, jobject activity)
{
    ...
    // HavanaRp /flymods - smali-defined methods on NvEventQueueActivity.
    s_hideAllUI = env->GetMethodID(nvEventClass, "hideAllUI", "()V");
    s_showAllUI = env->GetMethodID(nvEventClass, "showAllUI", "()V");
...
void CJavaWrapper::HideAllUI()
{
    JNIEnv* env = GetEnv();
    if (!env || !s_hideAllUI) { return; }
    env->CallVoidMethod(this->activity, this->s_hideAllUI);
    EXCEPTION_CHECK(env);
}

void CJavaWrapper::ShowAllUI()
{
    JNIEnv* env = GetEnv();
    if (!env || !s_showAllUI) { return; }
    env->CallVoidMethod(this->activity, this->s_showAllUI);
    EXCEPTION_CHECK(env);
}
