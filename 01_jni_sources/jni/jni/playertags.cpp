...
void CPlayerTags::Render()
{
	...
	if(pNetGame && pNetGame->m_bShowPlayerTags)
	{
					...
					if(pPlayerPed->GetDistanceFromCamera() <= pNetGame->m_fNameTagDrawDistance)
					{
						...
						if (!pNetGame->m_bNameTagLOS || dwHitEntity)
						{
							if (pVoice)
							{
												...
												// [HARDEN] snprintf so a server-corrupted player name
												// can't overflow the 50-byte stack buffer and feed bad
												// strings into GTA's text engine.
												snprintf(szNickBuf, sizeof(szNickBuf), "%s (%d)", pPlayerPool->GetPlayerName(playerId), playerId);
												szNickBuf[sizeof(szNickBuf) - 1] = '\0';
							...
							{
								// [HARDEN] snprintf to prevent name overflow.
								snprintf(szNickBuf, sizeof(szNickBuf), "%s (%d)", pPlayerPool->GetPlayerName(playerId), playerId);
								szNickBuf[sizeof(szNickBuf) - 1] = '\0';
								Draw(&VecPos, szNickBuf,
									pPlayer->GetPlayerColor(),
		...
		for (int i = 0; i <= 6; i++)
		{
			...
			// [HARDEN] snprintf to prevent name overflow on the voice list.
			snprintf(szNickBuf, sizeof(szNickBuf), "%s (%d)", pPlayerPool->GetPlayerName(aVoiceSortTime[i].m_iPlayerID), aVoiceSortTime[i].m_iPlayerID);
			szNickBuf[sizeof(szNickBuf) - 1] = '\0';
...
void CPlayerTags::AddChatBubble(PLAYERID playerId, char* szText, uint32_t dwColor, float fDistance, uint32_t dwTime)
{
	if (m_bChatBubbleStatus[playerId])
	{
		...
		// [HARDEN] Bound-convert into the 1024-byte chat-bubble buffer; a
		// server-side bubble of >1KB could otherwise smash the heap and feed
		// corrupt UTF-8 / UTF-16 to GTA's font engine.
		if (m_pSzText[playerId] && m_pSzTextWithoutColors[playerId] && szText)
		{
			cp1251_to_utf8(m_pSzText[playerId], szText, 1020);
			m_pSzText[playerId][1023] = '\0';
			cp1251_to_utf8(m_pSzTextWithoutColors[playerId], szText, 1020);
			m_pSzTextWithoutColors[playerId][1023] = '\0';
	...
	// [HARDEN] Bound the conversion to 1020 bytes so the trailing UTF-8
	// continuation byte and the NUL still fit inside our 1024-byte buffer.
	if (szText)
	{
		cp1251_to_utf8(m_pSzText[playerId], szText, 1020);
		m_pSzText[playerId][1023] = '\0';
	...
	{
		m_pSzText[playerId][0] = '\0';
		m_pSzTextWithoutColors[playerId][0] = '\0';
	}
	FilterColors(m_pSzTextWithoutColors[playerId]);
	const char* pText = m_pSzTextWithoutColors[playerId];
	m_iOffset[playerId] = 0;
	...
	float fDist, float fHealth, float fArmour, bool bAfk, bool bVoice, bool bKeyboard)
{
	...
	char tempBuff[300];
	// [HARDEN] Bound-convert into tempBuff so a >300-byte composed nick
	// (e.g. nick + tag chars + UTF-8 expansion) can't smash the stack and
	// feed corrupt strings to ImGui::CalcTextSize / pGUI->RenderText.
	if (szName)
	{
		cp1251_to_utf8(tempBuff, szName, sizeof(tempBuff) - 4);
