...
private:
    ...
    // Chat scroll state (per-peer view): pixels scrolled UP from bottom.
    // 0 = newest message visible. The total max is computed each frame
    // from the rendered content height vs viewport height.
    float m_chatScrollY;
    float m_chatScrollMax;
    bool  m_chatDragging;
    float m_chatDragStartY;
    float m_chatDragStartScroll;
    int   m_chatLastMsgCount;

    // Press start position. Tracked once per press-down so taps only
    // fire when both press AND release happen on the same widget.
    // This also lets drag-to-scroll consume the gesture without
    // accidentally tripping any button underneath the release point.
    float m_pressStartX;
    float m_pressStartY;
};
