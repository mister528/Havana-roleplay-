                                       ...
                                       const char* title, const char* body)
{
    ...
    if (strncmp(title, "[!POLICE_HUD_ON]",  16) == 0)
    {
        ...
        m_lastMouseDown = false;
        if (pWantedOverlay)
            pWantedOverlay->SetAnchor(m_cfgX, m_cfgY, m_cfgSize, true);
        return true;
    }
    if (strncmp(title, "[!POLICE_HUD_OFF]", 17) == 0)
    {
        ...
        if (pWantedOverlay)
        {
            pWantedOverlay->SetAnchor(m_cfgX, m_cfgY, m_cfgSize, false);
            pWantedOverlay->SetIconEditMode(false);
        }
        return true;
    }
    if (strncmp(title, "[!POLICE_HUD_CFG]", 17) == 0)
    {
        ...
        if (ParseLayoutBody(body, x, y, s))
        {
            ...
            if (pWantedOverlay)
                pWantedOverlay->SetAnchor(m_cfgX, m_cfgY, m_cfgSize,
                                          m_bVisible);
        }
        return true;
    }
    ...
    {
        // Body is "CX CY CSZ" (legacy) or "CX CY CSZ WX WY WSZ" (new).
        float cx_v = m_cfgX, cy_v = m_cfgY, cs_v = m_cfgSize;
        float wx_v = 0.92f, wy_v = 0.62f, ws_v = 1.0f;
        if (pWantedOverlay)
            pWantedOverlay->GetIconAnchor(wx_v, wy_v, ws_v);
        if (body)
        {
            float a, b, c, d, e, f;
            int n = sscanf(body, "%f %f %f %f %f %f",
                           &a, &b, &c, &d, &e, &f);
            if (n == 6)
            {
                cx_v = a; cy_v = b; cs_v = c;
                wx_v = d; wy_v = e; ws_v = f;
            }
            else if (n == 3)
            {
                cx_v = a; cy_v = b; cs_v = c;
        ...
        m_cfgX = cx_v; m_cfgY = cy_v; m_cfgSize = cs_v;
        if (pWantedOverlay) pWantedOverlay->SetIconAnchor(wx_v, wy_v, ws_v);
        ...
        // Keep the wanted icon visible during edit mode (the player needs
        // to see what they're dragging) but hide the LIST PANEL.
        if (pWantedOverlay)
        {
            pWantedOverlay->Hide();
            pWantedOverlay->SetAnchor(m_cfgX, m_cfgY, m_cfgSize, true);
            pWantedOverlay->SetIconEditMode(true);
        }
        return true;
    }
    return false;
                                         ...
                                         float scale)
{
    ...
    if (RectButton(dl, "كلبشة +", leftX, baseY, btnW, btnH,
                   kColBtnBgN, kColBtnBgH, kColBtnBorder, kColBtnText,
                   mp, clickedThisFrame, 22.0f * scale))
    {
        m_cfgSize += 0.10f;
    ...
    // ---- size +/- buttons for the WANTED ICON (right side) --------------
    float rightX = io.DisplaySize.x - btnW - 36.0f * scale;
    float curWX = 0.92f, curWY = 0.62f, curWS = 1.0f;
    if (pWantedOverlay) pWantedOverlay->GetIconAnchor(curWX, curWY, curWS);
    if (font)
    {
        char sz[64];
        snprintf(sz, sizeof(sz), "المطلوب %.2f", curWS);
        float fs = 22.0f * scale;
        std::string s = Arabic::Shape(sz);
        ImVec2 ts = font->CalcTextSizeA(fs, FLT_MAX, 0.0f, s.c_str());
        float lx = rightX + (btnW - ts.x) * 0.5f;
        float ly = baseY - ts.y - 12.0f * scale;
        dl->AddText(font, fs, ImVec2(lx + 2, ly + 2),
                    IM_COL32(0, 0, 0, 230), s.c_str());
        dl->AddText(font, fs, ImVec2(lx, ly),
                    IM_COL32(255, 200, 100, 255), s.c_str());
    }
    if (RectButton(dl, "مطلوب +", rightX, baseY, btnW, btnH,
                   IM_COL32(60, 32, 18, 240), IM_COL32(120, 60, 30, 240),
                   IM_COL32(255, 200, 100, 255), kColBtnText,
                   mp, clickedThisFrame, 22.0f * scale))
    {
        curWS += 0.10f;
        if (curWS > 2.0f) curWS = 2.0f;
        if (pWantedOverlay) pWantedOverlay->SetIconAnchor(curWX, curWY, curWS);
    }
    if (RectButton(dl, "مطلوب -",
                   rightX, baseY + btnH + 18.0f * scale,
                   btnW, btnH,
                   IM_COL32(60, 32, 18, 240), IM_COL32(120, 60, 30, 240),
                   IM_COL32(255, 200, 100, 255), kColBtnText,
                   mp, clickedThisFrame, 22.0f * scale))
    ...
    {
        float wx = 0.92f, wy = 0.62f, ws = 1.0f;
        if (pWantedOverlay) pWantedOverlay->GetIconAnchor(wx, wy, ws);
        char cmd[160];
        snprintf(cmd, sizeof(cmd), "/hudset %.4f %.4f %.4f %.4f %.4f %.4f",
                 m_cfgX, m_cfgY, m_cfgSize, wx, wy, ws);
                   ...
                   mp, clickedThisFrame, 32.0f * scale))
    {
        ...
        if (pWantedOverlay)
        {
            pWantedOverlay->SetIconAnchor(m_savedWX, m_savedWY, m_savedWSize);
            pWantedOverlay->SetAnchor(m_cfgX, m_cfgY, m_cfgSize, true);
            pWantedOverlay->SetIconEditMode(false);
        }
    }
...
void CPoliceHudOverlay::Render()
{
    ...
    if (m_editMode)
    {
        ...
        // ---- drag either icon ---------------------------------------
        // We hit-test cuff and wanted icon and, on press, latch onto
        // whichever is under the finger. The chosen target then keeps
        // tracking until the press ends.
        float wcx = 0.0f, wcy = 0.0f, wr = 0.0f;
        if (pWantedOverlay)
            pWantedOverlay->GetIconCenterPx(io.DisplaySize.x, io.DisplaySize.y,
                                            wcx, wcy, wr);
        bool insideCuff   = inside;
        float wdx = mp.x - wcx, wdy = mp.y - wcy;
        bool insideWanted = (pWantedOverlay && wr > 0.0f &&
                             (wdx*wdx + wdy*wdy) <= (wr * wr));

        if (mouseDown)
        {
            if (m_dragTarget == 0)
            {
                if (insideCuff)        m_dragTarget = 1;
                else if (insideWanted) m_dragTarget = 2;
            }
            if (m_dragTarget == 1)
            {
                ...
                m_cfgY = ny;
                if (pWantedOverlay)
                    pWantedOverlay->SetAnchor(m_cfgX, m_cfgY,
                                              m_cfgSize, true);
            }
            else if (m_dragTarget == 2 && pWantedOverlay)
            {
                float nx = mp.x / io.DisplaySize.x;
                float ny = mp.y / io.DisplaySize.y;
                if (nx < 0.05f) nx = 0.05f; if (nx > 0.95f) nx = 0.95f;
                if (ny < 0.05f) ny = 0.05f; if (ny > 0.95f) ny = 0.95f;
                float curWX, curWY, curWS;
                pWantedOverlay->GetIconAnchor(curWX, curWY, curWS);
                pWantedOverlay->SetIconAnchor(nx, ny, curWS);
            }
        }
        else
        {
    ...
    if (clicked)
    {
        // Cuff icon = original behaviour: cuff the nearest player. The
        // wanted icon is a separate floating button drawn by
        // wantedoverlay (independent X / Y / size).
