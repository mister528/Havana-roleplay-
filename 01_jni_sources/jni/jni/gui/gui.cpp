...
void CGUI::Render()
{
	...
	// Frame-tick for the WeaponCraft texture loader.  This is the ONLY
	// place RenderWare-backed PNG rasters are allowed to be created — never
	// inside an overlay's hot Render() path — to eliminate any first-show
	// race with RwImageAllocatePixels / RwRasterCreate.
	WeaponCraftTex::Tick();
    ...
    if (pWantedOverlay)    pWantedOverlay->Render();
    ...
    if (pWeaponCraftOverlay) pWeaponCraftOverlay->Render();
...
{
	// Build banner intentionally disabled in production builds.
}
