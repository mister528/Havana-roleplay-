	...
	float posX, float posY, float posZ, float drawDistance, bool useLOS, PLAYERID attachedToPlayerID, VEHICLEID attachedToVehicleID)
{
	...
	if (pTextLabel)
	{
		...
		// [HARDEN] Bound the cp1251 → UTF-8 conversion to (sizeof(text) - 8)
		// so the worst-case 3-byte UTF-8 expansion still fits and a NUL
		// terminator is guaranteed. text/textWithoutColors are 2049-byte
		// fixed buffers; an unterminated server-side label could otherwise
		// run cp1251_to_utf8 past the end and corrupt the heap, which the
		// game then trips over in CFont::GetNextSpace.
		if (text)
		{
			cp1251_to_utf8(pTextLabel->text, text, sizeof(pTextLabel->text) - 8);
			pTextLabel->text[sizeof(pTextLabel->text) - 1] = '\0';
			cp1251_to_utf8(pTextLabel->textWithoutColors, text, sizeof(pTextLabel->textWithoutColors) - 8);
			pTextLabel->textWithoutColors[sizeof(pTextLabel->textWithoutColors) - 1] = '\0';
		}
		else
		{
			pTextLabel->text[0] = '\0';
			pTextLabel->textWithoutColors[0] = '\0';
		}
...
void CText3DLabelsPool::Update3DLabel(int labelID, uint32_t color, char* text)
{
	...
	if (m_bSlotState[labelID] == true && m_pTextLabels[labelID])
	{
		...
		// [HARDEN] Bound the conversion as above (CreateTextLabel).
		if (text)
		{
			cp1251_to_utf8(m_pTextLabels[labelID]->text, text,
				sizeof(m_pTextLabels[labelID]->text) - 8);
			m_pTextLabels[labelID]->text[sizeof(m_pTextLabels[labelID]->text) - 1] = '\0';
		}
		else
		{
