...
#include "../gui/bankoverlay.h"
#include "../gui/carrentoverlay.h"
#include "../gui/trentpreviewoverlay.h"
#include "../gui/flymodsoverlay.h"
#include "../gui/shop247overlay.h"
#include "../gui/inventoryoverlay.h"
#include "../gui/houseguardoverlay.h"
#include "../gui/fishermanoverlay.h"
#include "../gui/ytmusicoverlay.h"
#include "../gui/clothesshopoverlay.h"
#include "../gui/accessoryshopoverlay.h"
#include "../gui/carshopoverlay.h"
#include "../gui/robberyoverlay.h"
#include "../gui/carjackoverlay.h"
#include "../gui/policehudoverlay.h"
#include "../gui/wantedoverlay.h"
#include "../gui/testinvoverlay.h"
#include "../gui/test3overlay.h"
#include "../gui/flightjoboverlay.h"
#include "../gui/phoneoverlay.h"
#include "../gui/objecteditoroverlay.h"
#include "../gui/islandtoolsoverlay.h"
#include "../gui/weaponcraftoverlay.h"
#include "../gui/gui.h"
#include <stdlib.h>
#include <string.h>
...
// Bridge for carrentoverlay.cpp, which cannot include the netgame.h chain.
void CarRent_SendDialogResponse(uint16_t id, uint8_t btn, uint16_t listitem, char* input)
{
    if (pNetGame) pNetGame->SendDialogResponse(id, btn, listitem, input);
}

// Bridge for houseguardoverlay.cpp (same reasoning).
void HouseGuard_SendDialogResponse(uint16_t id, uint8_t btn, uint16_t listitem, char* input)
{
    if (pNetGame) pNetGame->SendDialogResponse(id, btn, listitem, input);
}

// Bridge for fishermanoverlay.cpp (same reasoning).
void Fisherman_SendDialogResponse(uint16_t id, uint8_t btn, uint16_t listitem, char* input)
{
    if (pNetGame) pNetGame->SendDialogResponse(id, btn, listitem, input);
}

// Bridge for ytmusicoverlay.cpp (same reasoning).
void YtMusic_SendDialogResponse(uint16_t id, uint8_t btn, uint16_t listitem, char* input)
{
    if (pNetGame) pNetGame->SendDialogResponse(id, btn, listitem, input);
}

// Bridge for robberyoverlay.cpp (same reasoning).
void Robbery_SendDialogResponse(uint16_t id, uint8_t btn, uint16_t listitem, char* input)
{
    if (pNetGame) pNetGame->SendDialogResponse(id, btn, listitem, input);
}

void CarJack_SendDialogResponse(uint16_t id, uint8_t btn, uint16_t listitem, char* input)
{
    if (pNetGame) pNetGame->SendDialogResponse(id, btn, listitem, input);
}

void FlightJob_SendDialogResponse(uint16_t id, uint8_t btn, uint16_t listitem, char* input)
{
    if (pNetGame) pNetGame->SendDialogResponse(id, btn, listitem, input);
}

// Bridge for policehudoverlay.cpp - the HUD button issues a chat command
// instead of a dialog response so it can be tapped repeatedly without
// needing the server to re-send a dialog after every click.
void PoliceHud_SendChatCommand(const char* cmd)
{
    if (pNetGame && cmd && cmd[0]) pNetGame->SendChatCommand(cmd);
}

// Bridge for inventoryoverlay.cpp — sends a /command to the server when
// the player taps a usable item card in the bag overlay.
void Inventory_SendChatCommand(const char* cmd)
{
    if (pNetGame && cmd && cmd[0]) pNetGame->SendChatCommand(cmd);
}
...
{
    if (!pNetGame || !cmd || !cmd[0]) return;
    pNetGame->SendChatCommand(cmd);
    if (pGUI) pGUI->ResetTouchState();
}

void Phone_ShowLocalOverlay()
{
    if (pPhoneOverlay) pPhoneOverlay->ShowLocal();
}

// Bridges for inventoryoverlay.cpp (kept here so the overlay TU stays light).
const char* Inv_GetLocalPlayerName()
{
    if (pNetGame && pNetGame->GetPlayerPool())
        return pNetGame->GetPlayerPool()->GetLocalPlayerName();
    return nullptr;
}
int Inv_GetLocalSkinId()
{
    if (pGame && pGame->FindPlayerPed())
        return (int)pGame->FindPlayerPed()->GetModelIndex();
    return -1;
}
int Inv_GetLocalPlayerScore()
{
    if (pNetGame && pNetGame->GetPlayerPool())
        return pNetGame->GetPlayerPool()->GetLocalPlayerScore();
    return 0;
}
...
void InitGame(RPCParameters *rpcParams)
{
	...
	if(pChatWindow) pChatWindow->AddDebugMessage("تم الاتصال بـ {B9C9BF}%.64s", pNetGame->m_szHostName);
}
...
void ClientMessage(RPCParameters *rpcParams)
{
	...
	// [BANK OVERLAY v11] Intercept custom bank UI message from server.
	// Format: "~BANK_UI~<cash>|<bank>"  (e.g. "~BANK_UI~12345|987654")
	// The message is consumed locally (not shown in chat) and triggers the overlay.
	static const char kBankPrefix[] = "~BANK_UI~";
	const size_t kBankPrefixLen = sizeof(kBankPrefix) - 1;
	if (dwStrLen >= kBankPrefixLen && strncmp(szMsg, kBankPrefix, kBankPrefixLen) == 0)
	{
		const char* p = szMsg + kBankPrefixLen;
		char* sep = strchr((char*)p, '|');
		if (sep && pBankOverlay)
		{
			*sep = '\0';
			int64_t cash = strtoll(p, nullptr, 10);
			int64_t bank = strtoll(sep + 1, nullptr, 10);
			Log("[BANK_UI] show cash=%lld bank=%lld", (long long)cash, (long long)bank);
			pBankOverlay->Show(cash, bank);
		}
		free(szMsg);
		return;
	}

	// [INVENTORY OVERLAY] Intercept custom inventory UI messages from server.
	//   "~INV_UI~k=v;..."   - legacy: parse + immediately show overlay.
	//   "~INV_DATA~k=v;..." - silent: merge fields into the overlay's cache,
	//                        do not show. Used by inv_ui.pwn to push fresh
	//                        wallet data periodically so the overlay always
	//                        has accurate cash/bank/phone/mats/cigs values
	//                        when the player types /inv.
	// In both cases the message is consumed locally (never shown in chat).
	static const char kInvPrefix[]  = "~INV_UI~";
	static const char kInvDataPfx[] = "~INV_DATA~";
	const size_t kInvPrefixLen  = sizeof(kInvPrefix)  - 1;
	const size_t kInvDataPfxLen = sizeof(kInvDataPfx) - 1;
	if (dwStrLen >= kInvDataPfxLen && strncmp(szMsg, kInvDataPfx, kInvDataPfxLen) == 0)
	{
		const char* payload = szMsg + kInvDataPfxLen;
		if (pInventoryOverlay) pInventoryOverlay->UpdateFromPayload(payload);
		if (pTestInvOverlay) pTestInvOverlay->SetInventoryPayload(payload);
		free(szMsg);
		return;
	}
	if (dwStrLen >= kInvPrefixLen && strncmp(szMsg, kInvPrefix, kInvPrefixLen) == 0)
	{
		const char* payload = szMsg + kInvPrefixLen;
		if (pInventoryOverlay)
		{
			pInventoryOverlay->UpdateFromPayload(payload);
			pInventoryOverlay->Hide();
		}
		if (pTestInvOverlay) pTestInvOverlay->ShowFromPayload(payload);
		free(szMsg);
		return;
	}


	static const char kIslandInvPrefix[] = "~ISLANDINV~";
	const size_t kIslandInvPrefixLen = sizeof(kIslandInvPrefix) - 1;
	if (dwStrLen >= kIslandInvPrefixLen && strncmp(szMsg, kIslandInvPrefix, kIslandInvPrefixLen) == 0)
	{
		const char* payload = szMsg + kIslandInvPrefixLen;
		if (pInventoryOverlay) pInventoryOverlay->UpdateFromPayload(payload);
		if (pTestInvOverlay) pTestInvOverlay->SetInventoryPayload(payload);
		free(szMsg);
		return;
	...
	static const char kIslandToolPrefix[] = "~ISLANDTOOL~";
	const size_t kIslandToolPrefixLen = sizeof(kIslandToolPrefix) - 1;
	if (dwStrLen >= kIslandToolPrefixLen && strncmp(szMsg, kIslandToolPrefix, kIslandToolPrefixLen) == 0)
	{
		const char* payload = szMsg + kIslandToolPrefixLen;
		if (!strncmp(payload, "hide", 4))
		{
			if (pIslandToolsOverlay) pIslandToolsOverlay->Hide();
			free(szMsg);
			return;
		}
		int progress = 0;
		...
		if (buf)
		{
			...
			while (tok)
			{
				...
				{
					*eq = '\0';
					if (!strcmp(tok, "progress")) progress = atoi(eq + 1);
					else if (!strcmp(tok, "name")) { strncpy(name, eq + 1, sizeof(name) - 1); name[sizeof(name) - 1] = '\0'; }
				}
				tok = strtok_r(nullptr, ";", &save);
			}
			free(buf);
		}
		if (pIslandToolsOverlay) pIslandToolsOverlay->Show(progress, name);
		free(szMsg);
		return;
	}

	// [ACC_UI] Accessories: "~ACC_UI~s0=<model>,<used>,<name>;s1=...;"
	// Sent alongside INV_UI to populate the bag overlay's accessories grid.
	static const char kAccPrefix[] = "~ACC_UI~";
	const size_t kAccPrefixLen = sizeof(kAccPrefix) - 1;
	if (dwStrLen >= kAccPrefixLen && strncmp(szMsg, kAccPrefix, kAccPrefixLen) == 0)
	{
		if (pInventoryOverlay)
		{
			pInventoryOverlay->ClearAccessorySlots();
			if (pTestInvOverlay) pTestInvOverlay->ClearAccessorySlots();
			char* buf = strdup(szMsg + kAccPrefixLen);
			if (buf)
			{
				char* save = nullptr;
				char* tok  = strtok_r(buf, ";", &save);
							...
							*eq = '\0';
							...
							if (c1)
							{
								*c1 = '\0';
								int model = atoi(rest);
								char* rest2 = c1 + 1;
								char* c2 = strchr(rest2, ',');
								int used = 0;
								const char* name = "";
								if (c2) { *c2 = '\0'; used = atoi(rest2); name = c2 + 1; }
								else    { used = atoi(rest2); }
								pInventoryOverlay->SetAccessorySlot(slot, model, used, name);
								if (pTestInvOverlay) pTestInvOverlay->SetAccessorySlot(slot, model, used, name);
							}
						}
					}
					tok = strtok_r(nullptr, ";", &save);
	...
	// [FLYMODS] cinematic admin camera. Server (flymods.pwn) authorizes admin>=8
	// then pushes "~FLYMODS~ON|x|y|z", "~FLYMODS~OFF", or "~FLYMODS~POS|x|y|z|lx|ly|lz".
	// The launcher hides every HUD element and drives the GTA SA camera locally.
	static const char kFlyPrefix[] = "~FLYMODS~";
	const size_t kFlyPrefixLen = sizeof(kFlyPrefix) - 1;
	if (dwStrLen >= kFlyPrefixLen && strncmp(szMsg, kFlyPrefix, kFlyPrefixLen) == 0)
	{
		const char* payload = szMsg + kFlyPrefixLen;
		if (pFlyModsOverlay)
		{
			Log("[FLYMODS] payload=%s", payload);
			pFlyModsOverlay->ApplyServerCommand(payload);
		}
		free(szMsg);
		return;
	}

	static const char kObjEditPrefix[] = "~OBJEDIT~";
	const size_t kObjEditPrefixLen = sizeof(kObjEditPrefix) - 1;
	if (dwStrLen >= kObjEditPrefixLen && strncmp(szMsg, kObjEditPrefix, kObjEditPrefixLen) == 0)
	{
		const char* payload = szMsg + kObjEditPrefixLen;
		if (pObjectEditorOverlay)
			pObjectEditorOverlay->ApplyServerCommand(payload);
		free(szMsg);
		return;
	}

	static const char kFlightBoxPrefix[] = "~FLIGHTWORK~BOX";
	const size_t kFlightBoxPrefixLen = sizeof(kFlightBoxPrefix) - 1;
	if (dwStrLen >= kFlightBoxPrefixLen && strncmp(szMsg, kFlightBoxPrefix, kFlightBoxPrefixLen) == 0)
	{
		if (pTest3Overlay) pTest3Overlay->ShowFlightBox();
		free(szMsg);
		return;
	}

	// [CRAFT_UI] Weapon-crafting overlay (improve_craftgun upgrade).
	// Server (arabonline.pwn) drives the overlay through these payloads:
	//   "~CRAFT_UI~OPEN|<spring>|<iron>|<scrap>|<gunpowder>|<wood>"
	//   "~CRAFT_UI~PROG|<percent>"
	//   "~CRAFT_UI~DONE"
	//   "~CRAFT_UI~HIDE"
	static const char kCraftPrefix[] = "~CRAFT_UI~";
	const size_t kCraftPrefixLen = sizeof(kCraftPrefix) - 1;
	if (dwStrLen >= kCraftPrefixLen && strncmp(szMsg, kCraftPrefix, kCraftPrefixLen) == 0)
	{
		const char* payload = szMsg + kCraftPrefixLen;
		if (pWeaponCraftOverlay)
		{
			if (strncmp(payload, "OPEN", 4) == 0)
			{
				const char* p = payload + 4;
				if (*p == '|') ++p;
				int spring = 0, iron = 0, scrap = 0, gun = 0, wood = 0;
				int parsed = sscanf(p, "%d|%d|%d|%d|%d", &spring, &iron, &scrap, &gun, &wood);
				if (parsed >= 5)
				{
					Log("[CRAFT_UI] OPEN s=%d i=%d sc=%d g=%d w=%d",
					    spring, iron, scrap, gun, wood);
					pWeaponCraftOverlay->ShowList(spring, iron, scrap, gun, wood);
				}
			}
			else if (strncmp(payload, "PROG", 4) == 0)
			{
				const char* p = payload + 4;
				if (*p == '|') ++p;
				int pct = atoi(p);
				pWeaponCraftOverlay->ShowProgress(pct);
			}
			else if (strncmp(payload, "DONE", 4) == 0)
			{
				pWeaponCraftOverlay->ShowDone();
			}
			else if (strncmp(payload, "HIDE", 4) == 0)
			{
				pWeaponCraftOverlay->Hide();
			}
		}
		free(szMsg);
		return;
	...
	static const char kPhonePrefix[] = "~PHONE_UI~";
	const size_t kPhonePrefixLen = sizeof(kPhonePrefix) - 1;
	if (dwStrLen >= kPhonePrefixLen && strncmp(szMsg, kPhonePrefix, kPhonePrefixLen) == 0)
	{
		const char* payload = szMsg + kPhonePrefixLen;
		if (pPhoneOverlay)
		{
			if (strcmp(payload, "SHOW") == 0)
				pPhoneOverlay->ShowLocal();
			else
				pPhoneOverlay->ApplyServerCommand(payload);
		}
		free(szMsg);
		return;
	}

	// [HAV_YTM] spatial-audio broadcast.  Format:
	//   "[HAV_YTM]PLAY:<playerid>:<trackIdx>"
	//   "[HAV_YTM]STOP:<playerid>"
	// The gamemode rebroadcasts these on /_yt and /_ytstop. Each client
	// downloads the same /top10/<idx> mp3 and plays it locally with a
	// distance-based volume falloff.
	static const char kYtmPrefix[] = "[HAV_YTM]";
	const size_t kYtmPrefixLen = sizeof(kYtmPrefix) - 1;
	if (dwStrLen >= kYtmPrefixLen &&
	    strncmp(szMsg, kYtmPrefix, kYtmPrefixLen) == 0)
	{
		const char* p = szMsg + kYtmPrefixLen;
		if (pYtMusicOverlay)
		{
			if (strncmp(p, "PLAY:", 5) == 0)
			{
				const char* q = p + 5;
				int pid = atoi(q);
				const char* sep = strchr(q, ':');
				int idx = (sep ? atoi(sep + 1) : 0);
				if (idx > 0)
					pYtMusicOverlay->HandleRemotePlay(pid, idx);
			}
			else if (strncmp(p, "STOP:", 5) == 0)
			{
				int pid = atoi(p + 5);
				pYtMusicOverlay->HandleRemoteStop(pid);
			}
		}
		free(szMsg);
		return;
	...
	if (pNetGame && pNetGame->GetPlayerPool())
		CmdSuggest::OnIncomingClientMessage(szMsg, pNetGame->GetPlayerPool()->GetLocalPlayerName());
...
void DialogBox(RPCParameters *rpcParams)
{
	...
	bsData.Read(wDialogID);
		...
		// title
		// [HARDEN] Each cp1251 byte can expand to up to 3 UTF-8 bytes; cap
		// the input length so the worst-case expansion still fits in the
		// destination (and force a NUL terminator on the destination).  A
		// long server-side title/button used to overflow the 21-byte
		// button locals and the 193-byte title buffers, smashing the stack
		// and the dialog object on the heap, which then trapped GTA-SA's
		// strncpy / CFont::GetNextSpace later when the dialog rendered.
		bsData.Read(len);
		if (len > sizeof(szBuff) - 1) len = sizeof(szBuff) - 1;
		bsData.Read(szBuff, len);
		szBuff[len] = '\0';
		cp1251_to_utf8(title, szBuff, (sizeof(title) - 1) / 3);
		title[sizeof(title) - 1] = '\0';
		cp1251_to_utf8(pDialogWindow->m_utf8Title, szBuff, (sizeof(pDialogWindow->m_utf8Title) - 1) / 3);
		pDialogWindow->m_utf8Title[sizeof(pDialogWindow->m_utf8Title) - 1] = '\0';
		// button1
		bsData.Read(len);
		if (len > sizeof(szBuff) - 1) len = sizeof(szBuff) - 1;
		bsData.Read(szBuff, len);
		szBuff[len] = '\0';
		cp1251_to_utf8(button1, szBuff, (sizeof(button1) - 1) / 3);
		button1[sizeof(button1) - 1] = '\0';
		cp1251_to_utf8(pDialogWindow->m_utf8Button1, szBuff, (sizeof(pDialogWindow->m_utf8Button1) - 1) / 3);
		pDialogWindow->m_utf8Button1[sizeof(pDialogWindow->m_utf8Button1) - 1] = '\0';
		// button2
		bsData.Read(len);
		if (len > sizeof(szBuff) - 1) len = sizeof(szBuff) - 1;
		bsData.Read(szBuff, len);
		szBuff[len] = '\0';
		cp1251_to_utf8(button2, szBuff, (sizeof(button2) - 1) / 3);
		button2[sizeof(button2) - 1] = '\0';
		cp1251_to_utf8(pDialogWindow->m_utf8Button2, szBuff, (sizeof(pDialogWindow->m_utf8Button2) - 1) / 3);
		pDialogWindow->m_utf8Button2[sizeof(pDialogWindow->m_utf8Button2) - 1] = '\0';

		// info
		// [HARDEN] DecodeString writes up to 4096 cp1251 bytes; force NUL
		// terminator on szBuff and bound the cp1251 → UTF-8 expansion to
		// (sizeof(info) - 1) / 3 input bytes so a 4096-byte cyrillic info
		// can't expand to 8192+ bytes and overflow the 4097-byte info
		// stack buffer.
		stringCompressor->DecodeString(szBuff, 4096, &bsData);
		szBuff[sizeof(szBuff) - 1] = '\0';
		cp1251_to_utf8(info, szBuff, (sizeof(info) - 1) / 3);
		info[sizeof(info) - 1] = '\0';
		pDialogWindow->SetInfo(szBuff, strlen(szBuff));

		if(wDialogID < 0) return;

		// [TRENT PREVIEW] Sentinel dialog "[!TRENT_PREVIEW_OPEN]" enters
		// camera-only showroom preview. Any other incoming dialog
		// dismisses the preview overlay if it was active.
		if (pTrentPreviewOverlay)
		{
			if (pTrentPreviewOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
				return;
			pTrentPreviewOverlay->OnAnyDialog();
		}

		// [CLOTHES SHOP] Sentinel "[!CLOTHES_SHOP_OPEN]" / "[!CLOTHES_SHOP_CLOSE]".
		if (pClothesShopOverlay)
		{
			if (pClothesShopOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
				return;
			pClothesShopOverlay->OnAnyDialog();
		}

		// [ACCESSORY SHOP] Sentinel "[!ACC_SHOP_OPEN]" / "[!ACC_SHOP_CLOSE]".
		if (pAccessoryShopOverlay)
		{
			if (pAccessoryShopOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
				return;
			pAccessoryShopOverlay->OnAnyDialog();
		}

		// [CAR SHOP] Sentinel "[!CARSHOP_OPEN]" / "[!CARSHOP_CLOSE]".
		if (pCarShopOverlay)
		{
			if (pCarShopOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
				return;
			pCarShopOverlay->OnAnyDialog();
		}

		// [CAR RENTAL v12] Intercept rental dialogs and render as a fancy overlay
		// instead of the stock dialog. The overlay calls SendDialogResponse on
		// user action, so the gamemode's existing flow proceeds unchanged.
		if (pCarRentOverlay && pCarRentOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
			return;

		// [SHOP 24/7] Render 24/7 market with a polished overlay instead of
		// the stock list dialog. Uses the same SendDialogResponse bridge.
		if (pShop247Overlay && pShop247Overlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
			return;

		// [INVENTORY OVERLAY] Render the gamemode's "Inventory" SAMP dialog
		// as a wide professional overlay. Auto-cancels the dialog server-side.
		if (pInventoryOverlay && pInventoryOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
			return;

		// [HOUSE GUARD v12] Intercept the lock-picking dialog and replace
		// it with a circular skill minigame. The overlay sends a single
		// SendDialogResponse(id, 1, 0, "") on success or (id, 0, 0, "")
		// on cancel/give-up so the gamemode advances normally.
		if (pHouseGuardOverlay && pHouseGuardOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
			return;

		if (pFlightJobOverlay && pFlightJobOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
			return;

		// [FISHERMAN v13] Intercept dialogs coming from fisherman.pwn
		// (titles "HAVANA_FISH_*") and render them as an underwater
		// marine-themed overlay (shop laptop / boat rental / cast
		// minigame / end-of-job stats).
		if (pFishermanOverlay && pFishermanOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
			return;

		// [YTMUSIC] Intercept dialogs whose title starts with "HAVANA_YTMUSIC"
		// and replace them with the slide-up music player overlay.
		if (pYtMusicOverlay && pYtMusicOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
			return;

		// [ROBBERY 24/7] Sentinel "[!ROB247_OPEN]" / "[!ROB247_CLOSE]".
		if (pRobberyOverlay && pRobberyOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
			return;

		if (pCarJackOverlay && pCarJackOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
			return;

		// [POLICE HUD] "[!POLICE_HUD_ON]" / "[!POLICE_HUD_OFF]".
		if (pPoliceHudOverlay && pPoliceHudOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
			return;

		// [WANTED LIST] "[!POLICE_HUD_LIST]" / "[!POLICE_HUD_INFO]".
		if (pWantedOverlay && pWantedOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
			return;

		if (pTestInvOverlay && pTestInvOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
			return;

		if (pPhoneOverlay && pPhoneOverlay->TryHandleDialog(wDialogID, byteDialogStyle, title, info))
			return;
...
{
	pChatWindow->AddInfoMessage("السيرفر يعيد التشغيل.. انتظر شوي..");
...
void ConnectionRejected(RPCParameters *rpcParams)
{
	...
	if(byteRejectReason == REJECT_REASON_BAD_VERSION)
		pChatWindow->AddInfoMessage("تم رفض الاتصال: إصدار اللعبة غير صحيح.");
	else if(byteRejectReason == REJECT_REASON_BAD_NICKNAME)
	{
		pChatWindow->AddInfoMessage("تم إغلاق الاتصال: الاسم غير صحيح");
		pChatWindow->AddInfoMessage("اسم اللاعب لازم يكون من 3 إلى 20 حرف");
		pChatWindow->AddInfoMessage("استخدم فقط: a-z, A-Z, 0-9");
		pChatWindow->AddInfoMessage("اخرج من اللعبة وغيّر الاسم من اللانشر!");
	}
	else if(byteRejectReason == REJECT_REASON_BAD_MOD)
	{
		pChatWindow->AddInfoMessage("تم رفض الاتصال: إصدار المود غير صحيح.");
	}
	else if(byteRejectReason == REJECT_REASON_BAD_PLAYERID)
	{
		pChatWindow->AddInfoMessage("تم رفض الاتصال: ما فيه خانة لاعب متاحة.");
	}
