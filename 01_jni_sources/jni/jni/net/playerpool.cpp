...
{
	// [HARDEN] Range-check the slot first.  A corrupt or out-of-range
	// player ID must not index past m_pPlayers / m_szPlayerNames.
	...
	{
		// [HARDEN] Bound-copy the incoming nick into the 25-byte slot
		// (MAX_PLAYER_NAME + 1) and force a NUL terminator.  The previous
		// unchecked strcpy() let a long / non-terminated server-side nick
		// smash adjacent player-name slots; the corrupt strings later
		// trapped inside libGTASA's strncpy / CFont::GetNextSpace when
		// the radar / map / HUD tried to render them.
		if (szPlayerName)
		{
			size_t n = 0;
			while (szPlayerName[n] && n < MAX_PLAYER_NAME)
			{
				m_szPlayerNames[playerId][n] = szPlayerName[n];
				++n;
			}
