...
CNetGame::CNetGame(const char* szHostOrIp, int iPort, const char* szPlayerName, const char* szPass)
{
	...
	if(pChatWindow) pChatWindow->AddDebugMessage("جاري تحميل لاكشري...");
}
...
void CNetGame::Process()
{
	...
	if (m_pPlayerPool)
	{
		...
		// Deferred interior re-apply after teleports (see ScrSetPlayerPos).
	// Runs for a few frames so the streamer has time to bind interior meshes.
	{
		extern volatile int   g_iPendingInteriorFrames;
		extern volatile float g_fPendingInteriorX;
		extern volatile float g_fPendingInteriorY;
		if (g_iPendingInteriorFrames > 0)
		{
			if (m_pPlayerPool && m_pPlayerPool->GetLocalPlayer()
				&& m_pPlayerPool->GetLocalPlayer()->GetPlayerPed())
			{
				m_pPlayerPool->GetLocalPlayer()->GetPlayerPed()
					->ReapplyInteriorAt(g_fPendingInteriorX, g_fPendingInteriorY);
			}
			--g_iPendingInteriorFrames;
		}
	}
...
void CNetGame::UpdateNetwork()
{
	...
	while(pkt = m_pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			...
			pChatWindow->AddDebugMessage("فشل تهيئة التشفير.");
			break;
			...
			pChatWindow->AddDebugMessage("كلمة المرور غير صحيحة، ما تقدر تدخل السيرفر.");
			m_pRakClient->Disconnect(0);
			break;
...
void CNetGame::Packet_CustomRPC(Packet* p)
{
	...
	case RPC_OPEN_SETTINGS:
	{
		...
		case CUSTOM_RPC_TOGGLE_HUD_ELEMENT:
		{
			...
			bs.Read(hud);
			bs.Read(toggle);
			// [FIX] Ignore server requests to hide the vanilla SA-MP HUD
			// (money/weapon/ammo/wanted). User wants the stock SA-MP HUD
			// visible regardless of what the RP server tries to disable.
			if (hud == HUD_ELEMENT_HUD)
				toggle = 1;
...
void CNetGame::SendChatCommand(const char* szCommand)
{
	...
	m_pRakClient->RPC(&RPC_ServerCommand, &bsParams, HIGH_PRIORITY, RELIABLE_ORDERED, 0, false, UNASSIGNED_NETWORK_ID, NULL);
}
