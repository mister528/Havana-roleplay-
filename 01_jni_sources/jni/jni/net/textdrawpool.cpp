...
static bool IsGreenZoneTextdraw(CTextDraw* td)
{
	if (!td) return false;
	const char* text = td->GetText();
	if (!text) return false;
	if (!strcmp(text, "GREEN_ZONE")) return true;
	if (!strcmp(text, "NO_DAMAGE_HERE")) return true;
	if (td->m_TextDrawData.dwStyle == 5 && td->m_TextDrawData.wModelID == 3090) return true;
	if (td->m_TextDrawData.dwStyle == 4 && strstr(text, "hud:")) return true;
	if (td->m_TextDrawData.dwStyle == 4 && strstr(text, "LD_SPAC:white") &&
		td->m_TextDrawData.fX >= 10.0f && td->m_TextDrawData.fX <= 20.0f &&
		td->m_TextDrawData.fY >= 205.0f && td->m_TextDrawData.fY <= 215.0f) return true;
	return false;
}
...
{
	int x = 0;
	bool greenZoneShown = false;

	while (x != MAX_TEXT_DRAWS) {
		if (m_bSlotState[x])
		{
			if (IsGreenZoneTextdraw(m_pTextDraw[x]))
			{
				greenZoneShown = true;
				x++;
				continue;
	...
	if (pGreenZoneOverlay) pGreenZoneOverlay->SetActive(greenZoneShown);
}
