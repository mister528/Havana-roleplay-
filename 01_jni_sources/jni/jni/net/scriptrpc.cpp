...
// Pending interior re-apply. Processed in CNetGame::Process() over a few frames
// after a teleport so interior streaming settles at the new coordinates.
volatile int   g_iPendingInteriorFrames = 0;
volatile float g_fPendingInteriorX = 0.0f;
volatile float g_fPendingInteriorY = 0.0f;
...
void ScrSetPlayerPos(RPCParameters *rpcParams)
{
	...
	Log("RPC: ScrSetPlayerPos(%.2f, %.2f, %.2f)", vecPos.X, vecPos.Y, vecPos.Z);

	if(pLocalPlayer) pLocalPlayer->GetPlayerPed()->TeleportTo(vecPos.X,vecPos.Y,vecPos.Z);

	// Re-apply the current interior at the new coords. Calling the full
	// sequence (select_interior + link_actor_to_interior + refresh_streaming_at)
	// once is enough when the player has truly moved into the interior.
	if (pLocalPlayer && pLocalPlayer->GetPlayerPed())
		pLocalPlayer->GetPlayerPed()->ReapplyInteriorAt(vecPos.X, vecPos.Y);

	// No deferred frame loop: hammering select_interior every frame seems to
	// confuse the streamer on some interiors (observed for 24/7 / int 18).
	g_iPendingInteriorFrames = 0;
}
...
void ScrSetCameraPos(RPCParameters *rpcParams)
{
	...
	bsData.Read(vecPos.Y);
	bsData.Read(vecPos.Z);
	Log("RPC: ScrSetCameraPos(%.2f, %.2f, %.2f)", vecPos.X, vecPos.Y, vecPos.Z);
...
void ScrSetSpawnInfo(RPCParameters *rpcParams)
{
	...
	Log("RPC: ScrSetSpawnInfo skin=%d team=%d pos=(%.2f, %.2f, %.2f) rot=%.1f",
		SpawnInfo.iSkin, SpawnInfo.byteTeam,
		SpawnInfo.vecPos.X, SpawnInfo.vecPos.Y, SpawnInfo.vecPos.Z, SpawnInfo.fRotation);
...
void ScrSetPlayerInterior(RPCParameters *rpcParams)
{
	...
	Log("RPC: ScrSetPlayerInterior(%u)", (unsigned)byteInterior);

	pGame->FindPlayerPed()->SetInterior(byteInterior);

	// Some servers send SetPlayerInterior with the OLD player position (before
	// a paired SetPlayerPos). Remember the target interior so when the matching
	// teleport arrives, we can re-refresh streaming at the new location.
	// CPlayerPed::SetInterior already calls refresh_streaming_at with the
	// current matrix; we rely on ScrSetPlayerPos to refresh again after the
	// teleport takes effect.
}
...
void ScrCreateObject(RPCParameters* rpcParams)
{
	...
	if (iMaterialCount > 0)
	{
		...
		{
			uint16_t modelId = 0;
			uint8_t libLength = 0, texLength = 0;
			uint8_t id = 0;
			bsData.Read(id);
			if (id == 2) continue;
			uint8_t slot = 0;
			...
			// [FIX] cap at 255 to avoid oversized allocations, always null-
			// terminate after zero-initializing to prevent garbage strings
			// from leaking into GetTextureFromTXDStore (which crashes on bad
			// TXD/texture names inside libGTASA).
			char* str = new char[libLength + 1];
			memset(str, 0, libLength + 1);
			bsData.Read(str, libLength);
			str[libLength] = 0;

			bsData.Read(texLength);
			char* tex = new char[texLength + 1];
			memset(tex, 0, texLength + 1);
			...
			if (!pGame->IsModelLoaded(modelId))
			{
				...
				// [FIX] bounded wait; previously an infinite loop if the
				// model never loaded, which could also ANR the main thread.
				int iWait = 0;
				while (!pGame->IsModelLoaded(modelId) && iWait++ < 500)
					usleep(1000);
			}
			// [FIX] bounds-check slot to avoid writing past
			// m_pMaterials[MAX_MATERIALS], which was the most likely cause
			// of the CObjectPool-adjacent crash during gameplay.
			if (slot >= MAX_MATERIALS)
			{
				delete[] str;
				delete[] tex;
				// still drain the remaining bytes for this material
				uint32_t dwThrow;
				bsData.Read(dwThrow);
				continue;
			}
			...
			// [FIX] was m_pMaterials[i].dwColor (wrong index, could OOB) -
			// write into the same `slot` index used for the rest of the
			// material fields. Also delete[] the temp buffers to stop the
			// per-object memory leak.
			bsData.Read(pObject->m_pMaterials[slot].dwColor);
...
void ScrSetPlayerObjectMaterial(RPCParameters* rpcParams)
{
	...
	uint16_t wObjectID = 0;
	int16_t modelId = 0;
	uint8_t materialType = 0, matId = 0, libLength = 0, texLength = 0;
	RakNet::BitStream bsData(Data, (iBitLength / 8) + 1, false);
	bsData.Read(wObjectID);
	...
	bsData.Read(col);
	tex[texLength] = 0;
	// [FIX] guard against out-of-range matId and missing object pool entry
	// to stop the gameplay crash (was writing into m_pMaterials[matId] with
	// no bounds check -> trashed adjacent CObject slots).
	CObject* pObj = pNetGame->GetObjectPool()->GetAt(wObjectID);
	if (!pObj || matId >= MAX_MATERIALS) { delete[] str; delete[] tex; return; }
	...
	if (!pGame->IsModelLoaded(modelId))
	{
		pGame->RequestModel(modelId);
		pGame->LoadRequestedModels();
		// [FIX] bounded wait; was infinite loop that could hang the thread.
		int iWait = 0;
		while (!pGame->IsModelLoaded(modelId) && iWait++ < 500)
			usleep(1000);
	}
	...
	const char* pszTex = tex;
	if (!strncmp(tex, "materialtext1", sizeof("materialtext1")))
		pszTex = "MaterialText1";
	if (!strncmp(tex, "sampblack", sizeof("sampblack")))
		pszTex = "SAMPBlack";
	if (!strncmp(tex, "carpet19-128x128", sizeof("carpet19-128x128")))
		pszTex = "Carpet19-128x128";
	if (!pObj->m_pMaterials[matId].pTex && strncmp(pszTex, "none", sizeof("none"))
		&& strncmp(pszTex, "wall8", sizeof("wall8")))
	{
		pObj->m_pMaterials[matId].pTex = (RwTexture*)LoadTextureFromDB("samp", pszTex);
	}
	if (!GetModelReferenceCount(modelId) && pGame->IsModelLoaded(modelId))
	{
		ScriptCommand(&release_model, modelId);
	}
	// [FIX] plug the per-RPC memory leak - str/tex were allocated with new
	// but never deleted. Over thousands of material RPCs this leaks a lot.
	delete[] str;
	delete[] tex;
}
