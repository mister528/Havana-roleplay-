...
// [DEATH FIX] Track the last player/weapon that damaged us via bullet sync so
// that when our local player eventually dies, SendWastedNotification() can
// send the correct killer + weapon instead of INVALID_PLAYER_ID / 0. Without
// this, RP servers that require a valid killer to register a kill simply
// ignore the death RPC, so shooting another player never produced a death.
PLAYERID g_LastDamagerID     = INVALID_PLAYER_ID;
uint8_t  g_LastDamageWeapon  = 0;
uint32_t g_LastDamageTick    = 0;
...
void CRemotePlayer::StoreBulletSyncData(BULLET_SYNC* blSync)
{
	...
	pLocalPed->SetHealth(fHealth < 0.0f ? 0.0f : fHealth);

	// [DEATH FIX] Remember who shot us + with what. Used by
	// CLocalPlayer::SendWastedNotification() so the server can actually
	// register the kill instead of getting "killed by INVALID_PLAYER".
	g_LastDamagerID    = m_PlayerID;
	g_LastDamageWeapon = byteWeaponID;
	g_LastDamageTick   = GetTickCount();

	if (fHealth <= 0.0f) {
		// [DEATH FIX] Force the ped into the death state right away so the
		// local Process() loop picks up IsDead() on the next frame and
		// emits RPC_Death. Previously the death flag relied on the GTA
		// engine observing health==0 on its own schedule, which on some
		// weapons (especially low-damage continuous ones) never flipped
		// the action trigger to ACTION_DEATH, so the kill was never
