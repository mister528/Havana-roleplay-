...
void CStreamPool::PlayIndividualStream(const char* szUrl) // ready
{
	...
	// [HARDEN] m_szIndividualLastLink is 256 bytes; bound the copy and force
	// NUL terminator so a long server-side URL can't overflow into adjacent
	// fields (which would later corrupt strings GTA-SA prints).
	if (szUrl)
	{
		size_t n = 0;
		while (szUrl[n] && n < sizeof(m_szIndividualLastLink) - 1)
		{
			m_szIndividualLastLink[n] = szUrl[n];
			++n;
		}
		m_szIndividualLastLink[n] = '\0';
	}
	else
	{
		m_szIndividualLastLink[0] = '\0';
