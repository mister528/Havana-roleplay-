...
// [NPC FIX] Locate any remote-player-like entity (real player OR FCNPC bot)
// near the shot. Widened thresholds vs the old logic so Arena NPCs and
// similar bots - which don't always share the same GtaActor pointer as the
// ped the GTA engine damaged - still resolve to a valid playerid.
// The server's anti-cheat requires `hitType=ENTITY_TYPE_PED` with a real
// playerid in BULLET_SYNC in order to set player_legal_gun/player_legal_damage
// and allow the subsequent RPC_PlayerGiveTakeDamage to deal damage.
static PLAYERID FindRemotePlayerIDFromDamageContext(CPlayerPool* pPlayerPool, PED_TYPE* pDamaged, CAMERA_AIM* caAim)
{
	if (!pPlayerPool) return INVALID_PLAYER_ID;

	VECTOR damagedPos = { 0, 0, 0 };
	bool hasDamagedPos = false;
	if (pDamaged && pDamaged->entity.mat)
	{
		damagedPos = pDamaged->entity.mat->pos;
		hasDamagedPos = true;
	}

	// Fallback: local player ped matrix as aim origin if caAim is bogus.
	VECTOR aimOrigin = { 0, 0, 0 };
	VECTOR aimDir    = { 0, 0, 0 };
	bool hasAim = false;
	if (caAim)
	{
	...
	float    bestLine = 1000000.0f;   // best line-to-aim score (m^2)
	float    bestDist = 1000000.0f;   // best distance-to-pDamaged score (m^2)

	for (PLAYERID playerId = 0; playerId < MAX_PLAYERS; playerId++)
	{
		if (!pPlayerPool->GetSlotState(playerId)) continue;

		CRemotePlayer* pRemote = pPlayerPool->GetAt(playerId);
		if (!pRemote || !pRemote->GetPlayerPed()) continue;

		PED_TYPE* pRemotePed = pRemote->GetPlayerPed()->GetGtaActor();
		if (!pRemotePed || !pRemotePed->entity.mat) continue;

		// Direct pointer match (most reliable for real players)
		if (pRemotePed == pDamaged) return playerId;

		const VECTOR remotePos = pRemotePed->entity.mat->pos;

		// [NPC FIX] proximity test to pDamaged position, widened to 5m^2 = 25.
		// FCNPC bots: pDamaged may be an internal FCNPC ped pointer whose world
		// pos still matches the bot's visible remote-player pos within a few
		// meters.
		if (hasDamagedPos)
		{
			const float distanceScore = SquaredDistance(remotePos, damagedPos);
			if (distanceScore < 25.0f && distanceScore < bestDist)
			{
				bestDist = distanceScore;
				bestId   = playerId;
			}
		}

		// [NPC FIX] aim-cone test - widened line threshold from 1.5m to 3.5m
		// so slightly offset hits still resolve to the target.
		if (hasAim)
		{
			const VECTOR target = { remotePos.X, remotePos.Y, remotePos.Z + 0.8f };
			...
			if (forward > 0.0f && forward <= 250.0f)
			{
				...
				const float lineScore = SquaredDistance(target, closest);
				if (lineScore < 12.25f && lineScore < bestLine)
				{
					bestLine = lineScore;
					bestId   = playerId;
				}
...
static bool GetRemotePlayerAimHit(CPlayerPool* pPlayerPool, CAMERA_AIM* caAim, PLAYERID* pHitId, VECTOR* pHitPos)
{
	if (!pPlayerPool || !caAim || !pHitId || !pHitPos) return false;
	...
	for (PLAYERID playerId = 0; playerId < MAX_PLAYERS; playerId++)
	{
		...
		if (pRemote->GetState() == PLAYER_STATE_WASTED || pRemote->GetState() == PLAYER_STATE_NONE) continue;
...
stFile* NvFOpen_hook(const char* r0, const char* r1, int r2, int r3)
{
	...
	// Vehicle-patch: serve our shipped gta3.img (with Azelow dff/txd over
	// the STALLION slot) from the launcher's storage. The asset extractor
	// drops it there on first run after install.
	if (!strncmp(r1, "MODELS/GTA3.IMG", 15))
	{
		sprintf(path, "%s/SAMP/gta3.img", g_pszStorage);
		// Only redirect if the patched copy actually exists on disk;
		// otherwise the engine should fall through to the OBB original.
		if (access(path, R_OK) == 0)
		{
			Log("Loading patched gta3.img..");
			goto open;
		}
	}
	...
	if(pGame) 
	{
		if(pNetGame) 
		{
			if(pGame->FindPlayerPed() || GamePool_FindPlayerPed()) 
			{
				...
				// [HUD FIX] Always force the vanilla GTA HUD (money /
				// weapon box / wanted stars) visible. This branch used
				// to hide it when the user preferred the custom Java
				// overlay (iHud == 1), but servers can end up calling
				// ShowHud() right after spawn, which made the HUD
				// flash for a moment and then disappear. User wants
				// it always on.
...
{
	if(text && !strcmp(text, "MOB_LOCK"))
	{
		CFont::AsciiToGxtChar("Free Aim", gxt_string);
		return gxt_string;
	}

	if(text && text[0] == 'S' && text[1] == 'A' && text[2] == 'M' && text[3] == 'P')
	{
...
void TouchEvent_hook(int type, int num, int posX, int posY)
{
	...
	// ytmusic overlay is modal — feed touches to ImGui (so the overlay
	// buttons work) but never forward them to the game / chat / dialogs.
	if (pYtMusicOverlay && pYtMusicOverlay->IsVisible())
	{
	...
	{
		pGUI->OnTouchEvent(type, num, posX, posY);
		if (pKeyBoard && pKeyBoard->IsOpen() && !pPhoneOverlay->IsKeyboardActive())
			pKeyBoard->Close();
		return;
	}
	// 24/7 robbery lockpick overlay: fully modal during the lockpick phase,
	// non-modal during the post-success progress strip so the player can
	// watch the heist animation play out behind the small bottom HUD.
	if (pRobberyOverlay && pRobberyOverlay->IsVisible()
	    && !pRobberyOverlay->IsProgressMode())
	{
		pGUI->OnTouchEvent(type, num, posX, posY);
		return;
	}
	// Inventory overlay (bag / accessory popup / control pad) lets the
	// player keep moving the camera / walking around — only blocks the
	// chat (handled inside CChatWindow::OnTouchEvent). Touches inside the
	// overlay regions are consumed below by pGUI->OnTouchEvent.
...
{
	// [FIX] Call the stock CStreaming::InitImageList first so GTA:SA registers
	// its full set of IMG files (TEXDB\\GTA3.IMG, GTA_INT.IMG, TEXDB\\PLAYER.IMG,
	// ANIM\\CUTS.IMG, DATA\\SCRIPT\\SCRIPT.IMG, DATA\\SCRIPT\\SCRIPTV1.IMG,
	// MODELS\\GTA_INT.IMG). Wiping ms_files and re-adding only 4 caused the
	// whole base map (police station, buildings, interiors) to never stream.
	CStreaming_InitImageList();

	// Append SA-MP\'s extra IMG archives on top of the stock list so custom
	// server objects / collisions still work.
...
void ProcessPedDamage(PED_TYPE* pIssuer, PED_TYPE* pDamaged)
{
	...
	if (!pDamaged || pPedPlayer != pIssuer) return;

	// Never self-damage report
	if (pDamaged == pPedPlayer) return;

	CPlayerPool* pPlayerPool = pNetGame->GetPlayerPool();
	if (!pPlayerPool || !pPlayerPool->GetLocalPlayer() || !pPlayerPool->GetLocalPlayer()->GetPlayerPed()) return;

	CPlayerPed* pLocalPlayerPed = pPlayerPool->GetLocalPlayer()->GetPlayerPed();

	// [ACTOR FIX] Arena NPCs (and any CreateActor ped) are server-side Actors,
	// NOT players. Detect actor hits BEFORE trying the player pool. The damage
	// path for actors is an entirely different RPC (177 GiveActorDamage).
	CActorPool* pActorPool = pNetGame->GetActorPool();
	if (pActorPool)
	{
		VECTOR dpos = { 0, 0, 0 };
		bool  hasDPos = false;
		if (pDamaged->entity.mat)
		{
			dpos = pDamaged->entity.mat->pos;
			hasDPos = true;
		}

		int      bestActor     = -1;
		float    bestActorDist = 6.25f; // 2.5m^2 proximity fallback
		for (uint16_t aid = 0; aid < MAX_ACTORS; ++aid)
		{
			CActorPed* pActor = pActorPool->GetAt(aid);
			if (!pActor || !pActor->m_pPed) continue;

			// Exact pointer match is the fast path.
			if (pActor->m_pPed == pDamaged)
			{
				bestActor = (int)aid;
				bestActorDist = 0.0f;
				break;
			...
			// Fallback: match by world position — some GTA damage paths pass
			// a different PED_TYPE instance (internal vs streamed ped) so
			// pointer equality alone isn't sufficient.
			if (hasDPos && pActor->m_pPed->entity.mat)
			{
				const float d = SquaredDistance(pActor->m_pPed->entity.mat->pos, dpos);
				if (d < bestActorDist)
				{
					bestActorDist = d;
					bestActor = (int)aid;
		...
		if (bestActor >= 0)
		{
			const uint8_t weapon = pLocalPlayerPed->GetCurrentWeapon();
			pPlayerPool->GetLocalPlayer()->SendActorDamageNotification((uint16_t)bestActor, weapon);
			// Done — don't fall through to player BULLET_SYNC for an actor hit.
			return;
		}
	}

	PLAYERID remoteId = pPlayerPool->FindRemotePlayerIDFromGtaPtr((PED_TYPE*)pDamaged);

	CAMERA_AIM* caAim = pLocalPlayerPed->GetCurrentAim();
	if (remoteId == INVALID_PLAYER_ID)
		remoteId = FindRemotePlayerIDFromDamageContext(pPlayerPool, pDamaged, caAim);

	// [NPC FIX] Last-resort fallback: grab the closest remote player to the
	// damaged ped's world position, regardless of aim. FCNPC bots sometimes
	// leave the pool's GetSlotState=false briefly; extend the hunt to ALL
	// remote slots that have a valid ped, picking the one nearest (<= 8m).
	if (remoteId == INVALID_PLAYER_ID && pDamaged && pDamaged->entity.mat)
	{
		const VECTOR dpos = pDamaged->entity.mat->pos;
		float bestDist = 64.0f; // 8m squared
		for (PLAYERID pid = 0; pid < MAX_PLAYERS; ++pid)
		{
			CRemotePlayer* pr = pPlayerPool->GetAt(pid);
			if (!pr || !pr->GetPlayerPed()) continue;
			PED_TYPE* rp = pr->GetPlayerPed()->GetGtaActor();
			if (!rp || !rp->entity.mat) continue;
			const float d = SquaredDistance(rp->entity.mat->pos, dpos);
			if (d < bestDist)
			{
				bestDist = d;
				remoteId = pid;
	...
	VECTOR hitPos;
	if (remoteId != INVALID_PLAYER_ID && pPlayerPool->GetAt(remoteId) && pPlayerPool->GetAt(remoteId)->GetPlayerPed())
	{
		MATRIX4X4 remoteMat;
		pPlayerPool->GetAt(remoteId)->GetPlayerPed()->GetMatrix(&remoteMat);
		hitPos = remoteMat.pos;
		hitPos.Z += 0.8f;
	}
	else if (pDamaged && pDamaged->entity.mat)
	{
		hitPos = pDamaged->entity.mat->pos;
		hitPos.Z += 0.8f;
	}
	else if (caAim)
	{
		hitPos.X = caAim->pos1x + caAim->f1x * 100.0f;
		hitPos.Y = caAim->pos1y + caAim->f1y * 100.0f;
		hitPos.Z = caAim->pos1z + caAim->f1z * 100.0f;
	}
	else
	{
		return;
	...
	if (remoteId != INVALID_PLAYER_ID)
	{
		pPlayerPool->GetLocalPlayer()->SendBulletSyncData(remoteId, ENTITY_TYPE_PED, hitPos);
	}
	else
	{
		pPlayerPool->GetLocalPlayer()->SendBulletSyncData(INVALID_PLAYER_ID, ENTITY_TYPE_UNKNOWN, hitPos);
	}
}
...
{	
	NvUtilInit();
	*(char**)(g_libGTASA + 0x5D1608) = "/storage/emulated/0/LuxuryMobile/";
}
...
extern RwTexture* g_havanaBlackTexture; // splashscreen.cpp: opaque-black RwTexture
uintptr_t GetTexture_hook(const char* a1)
{
	// [HAVANA] Hide pause-menu side artwork ("Flin Roleplay" branding):
	// substitute a tiny opaque-black RwTexture for splash1/splash2/splash3.
	// (Defensive: ApplyPatches() also nulls these strings in libGTASA's
	// .rodata so the names should normally be "" by the time we get here.)
	if (a1 && g_havanaBlackTexture)
	{
		if (a1[0] == 's' && a1[1] == 'p' && a1[2] == 'l' && a1[3] == 'a' &&
		    a1[4] == 's' && a1[5] == 'h' &&
		    (a1[6] == '1' || a1[6] == '2' || a1[6] == '3') && a1[7] == '\0')
		{
			Log("[HavanaPauseArt] substituted black for %s", a1);
			++*(uintptr_t*)((uintptr_t)g_havanaBlackTexture + 84);
			return (uintptr_t)g_havanaBlackTexture;
		}
	}
	//001BE990
...
void CSprite2d__DrawBarChart_hook(float x, float y, unsigned short width, unsigned char height, float progress, signed char progressAdd,
	unsigned char drawPercentage, unsigned char drawBlackBorder, CRGBA* color, CRGBA* addColor)
{
	// [FIX] Launcher used to re-position / re-size / re-colour the HP/armor
	// bars to draw the custom HUD box. User wants the vanilla SA-MP bars
	// instead, so we forward the caller's parameters untouched.
	CSprite2d__DrawBarChart(x, y, width, height, progress, progressAdd, drawPercentage, drawBlackBorder, color, addColor);
	return;
...
// [HARDEN] Last-line-of-defense around CFont::GetNextSpace.  GTA-SA's
// implementation walks the uint16_t string forward looking for the next
// space (or terminator) and dereferences without bounds, so a non-terminated
// buffer (e.g. one fed via CMessages::AddBigMesssage from a corrupt server
// game-text, or a textdraw whose bytes leak past the 800-char window) will
// SIGSEGV inside GetNextSpace+22.  All input paths that reach this function
// (player names, chat, 3-D labels, textdraws, GameText) are now bound at
// the source, but we still hook the function to:
//   - reject NULL / unaligned pointers
//   - bail safely on strings that don't have a 0 terminator within 4096
//     code units (vastly more than any legit GTA-SA / SA-MP text)
// If the input is sane, we just call through to the original.
uint16_t* (*CFont__GetNextSpace)(uint16_t* str);
uint16_t* CFont__GetNextSpace_hook(uint16_t* str)
{
	if (!str || ((uintptr_t)str & 1)) return str;
	for (int i = 0; i < 4096; ++i)
	{
		uint16_t ch = str[i];
		if (ch == 0)
			return CFont__GetNextSpace(str);
		// Spaces (0x20) and tabs (0x09) are valid token boundaries, so we
		// don't filter them out.  Everything else under 0x20 except CR/LF
		// is treated as junk and we return the input unchanged so the
		// caller treats this as an empty token rather than crashing.
		if (ch < 0x20 && ch != ' ' && ch != '\n' && ch != '\r' && ch != '\t')
			return str;
	}
	// Couldn't find a terminator within the safe window; bail out.
	return str;
}

// [FIX] Stronger reachability check than the previous null/alignment guard.
// CFont::GetNextSpace walks the uint16_t string forward looking for a 0
// terminator and crashes with SIGSEGV if it walks off into unmapped memory
// (we have a real-world Redmi/Android-16 trace at CFont::GetNextSpace+22
// that comes through this hook with text non-null and word-aligned).
// mincore() returns 0 only when the page is mapped in our address space,
// so we use it to verify the text starts inside a mapped page and that we
// can find a 0 terminator within a reasonable bound (4096 chars - far more
// than any chat / textdraw / playername that GTA-SA would render).
static inline bool havana_text_safe(const uint16_t* text)
{
	if (!text || ((uintptr_t)text & 1))
		return false;
...
// [FIX] CFont::GetStringWidth at libGTASA + 0x5340A4 crashes when the
// map screen is opened and the engine tries to measure a zone/street name
// whose GXT text pointer is NULL or points to unmapped memory.
// Crash trace: CFont::GetStringWidth(unsigned short*, unsigned char,
// unsigned char, unsigned char)+26.  Hook the function and return 0
// (zero width) for any unsafe input so the HUD simply skips the label
// instead of crashing.
float (*CFont__GetStringWidth)(uint16_t* str, uint8_t bFull, uint8_t bUnk1, uint8_t bUnk2);
float CFont__GetStringWidth_hook(uint16_t* str, uint8_t bFull, uint8_t bUnk1, uint8_t bUnk2)
{
	if (!havana_text_safe(str))
		return 0.0f;
	return CFont__GetStringWidth(str, bFull, bUnk1, bUnk2);
}

void CFont__PrintString_hook(float x, float y, uint16_t* text)
{
	// [FIX] Guard against NULL / unaligned / unmapped text pointers before
	// passing them down into GTA:SA's native font code. See havana_text_safe
	// above for the mincore-based reachability probe.
	if (!havana_text_safe(text))
		return;

	// [FIX] Launcher used to re-locate / recolour the vanilla HUD text
	// (money, wanted stars, ammo) via CAdjustableHudPosition/Scale/Colors so
	// it could draw its own HUD box. User wants stock SA-MP HUD only, so we
	// short-circuit straight to the native implementation and let GTA:SA
	// render the text at the untouched coordinates the caller passed.
	CFont__PrintString(x, y, text);
	return;
...
{
	// [FIX] The launcher used to force CMobileHud offset to (45, 45), which
	// repositioned the money/ammo/weapon HUD towards the top-left corner so
	// the launcher could draw its own replacement "HUD box" over the radar.
	// Removing the override lets GTA:SA use its default offset, so the
	// vanilla SA-MP HUD (money top-right, weapon icon + ammo below it) is
	// rendered in its original position - same as stock SA-MP client.
	CHud__Draw();
}
...
void InstallHooks()
{
	...
	// [HARDEN] CFont::GetNextSpace at libGTASA + 0x533184 (crash trace was
	// at +22).  Reject malformed input before GTA's native code walks off
	// the end of the buffer.
	SetUpHook(g_libGTASA + 0x00533184, (uintptr_t)CFont__GetNextSpace_hook, (uintptr_t*)& CFont__GetNextSpace);
	// [FIX] CFont::GetStringWidth — crash when opening the map (zone names)
	SetUpHook(g_libGTASA + 0x005340A4, (uintptr_t)CFont__GetStringWidth_hook, (uintptr_t*)& CFont__GetStringWidth);
