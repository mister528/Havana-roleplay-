...
{
	// [HARDEN] szGameTextMessage is a fixed 1076 uint16_t buffer; an
	// unbounded AsciiToGxtChar() over a long server-side message can
	// overflow it (1 uint16_t per input byte plus expansion for colour /
	// keycode tokens), corrupt the heap, and trap GTA-SA later inside
	// CFont::GetNextSpace.  Reject NULL and clamp the input to a safe
	// length before any of the shaping / native-engine calls run.
	if (!szStr) return;
	char szSafeIn[1024];
	{
		size_t n = 0;
		while (szStr[n] && n < sizeof(szSafeIn) - 1) { szSafeIn[n] = szStr[n]; ++n; }
		szSafeIn[n] = '\0';
	}

	// Intercept Arabic-containing messages so we can shape + draw them via
	// ImGui (the GTA SA native text engine doesn't shape Arabic). Server
	// GameText uses the same legacy encoding path as chat/textdraws, so
	// normalise it before Arabic detection/shaping.
	char szUtf8[1024];
	// [HARDEN] Pass an explicit cap so cp1251_to_utf8 can't overflow szUtf8
	// even if its input lacks a NUL terminator.
	cp1251_to_utf8(szUtf8, szSafeIn, sizeof(szUtf8) - 4);
	szUtf8[sizeof(szUtf8) - 1] = '\0';
	if (pGameTextOverlay && pGameTextOverlay->TryShow(szUtf8, iTime, iType))
	{
		ScriptCommand(&text_clear_all);
		return;
	}
	ScriptCommand(&text_clear_all);
	CFont::AsciiToGxtChar(szSafeIn, szGameTextMessage);
	// [HARDEN] Force NUL terminator at the very end of the gxt buffer so
	// even if AsciiToGxtChar misbehaves, GetNextSpace has a hard stop.
	szGameTextMessage[1075] = 0;
