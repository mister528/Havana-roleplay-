...
static std::string BankShapeText(const char* s)
{
...
static ImVec2 BankCalcTextSize(ImFont* font, float size, const std::string& text)
{
    if (!font) return ImGui::CalcTextSize(text.c_str());
    return font->CalcTextSizeA(size, FLT_MAX, 0.0f, text.c_str());
}

static void BankAddText(ImDrawList* dl, ImFont* font, float size, ImVec2 pos,
                        ImU32 col, const char* text)
{
    if (!dl || !text) return;
    std::string shaped = BankShapeText(text);
...
void CBankOverlay::Render()
{
    ...
    if (pGUI && pGUI->GetFont())
    {
        ...
        BankAddText(dl, font, fSize,
                    ImVec2(pMin.x + 100.0f * scale, pMin.y + 24.0f * scale),
                    kColText, "بنك هافانا");
        BankAddText(dl, font, 18.0f * scale,
                    ImVec2(pMin.x + 100.0f * scale, pMin.y + 58.0f * scale),
                    kColTextDim, "ملخص الحساب");
    }
    else
    {
        BankAddText(dl, nullptr, 0.0f,
                    ImVec2(pMin.x + 100.0f * scale, pMin.y + 26.0f * scale),
                    kColText, "بنك هافانا");
    }
                       ...
                       ImU32 badgeCol, const char* badgeLetter)
    {
        ...
        if (font)
        {
            BankAddText(dl, font, 22.0f * scale,
                        ImVec2(rMin.x + 92.0f * scale, rMin.y + 16.0f * scale),
                        kColText, label);
            BankAddText(dl, font, 15.0f * scale,
    ...
    // Total
    {
        ...
        if (font)
        {
            BankAddText(dl, font, 20.0f * scale,
    ...
    if (font)
    {
        ...
        std::string hintText = BankShapeText(hint);
        ImVec2 tsz = BankCalcTextSize(font, fs, hintText);
        dl->AddText(font, fs,
                    ImVec2(pMin.x + (panelW - tsz.x) * 0.5f,
                           pMax.y - fs - 14.0f * scale),
                    kColTextMuted, hintText.c_str());
    }
